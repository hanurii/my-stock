...
원/달러 환율은 3/23 1,517원까지 치솟았다가 3/24 오전 1,489.5원(하나은행 공시)으로 소폭 안정됐다. 외국인 투자자 이탈과 수입물가 부담이 동시에 커지고 있다. 미 2년물 국채 수익률이 6월 이후 처음으로 4%를 돌파했다 — 유가발 인플레 우려로 금리 인하 기대가 후퇴하고 있다.
    ...
    fx: [
      {
        ...
        weekly_change: -1.38,
        trend: "▼ 원화 소폭 강세",
        comment: "하나은행 3/24 08:28 공시 기준 1,489.5원. 전일(1,510원대) 대비 하락.",
        timeseries: shiftTs(prev.indicators.fx[0].timeseries, "03/24", 1489.5),
      },
      {
        name: "달러인덱스",
        value: 99.10,
...
- 원/달러 환율이 3/23 **1,517원**까지 치솟은 뒤 3/24 오전 **1,489.5원**으로 소폭 안정.
...
> 🇰🇷 **한국 급락 괴리**: 코스피 5,406은 3/20(5,781) 대비 -6.5%. 미국은 반등했는데 한국만 폭락한 것은 원화 약세(1,517원)와 외국인 매도가 추가 변수로 작용한 것. 미국 반등 효과 반영 시 기술적 반등 여지 있으나, 환율 불안 지속 시 제한적.`,
  ...
  historical: prev.historical.map((h: Record<string, unknown>) => {
    ...
    if (h.name === "원/달러") return { ...h, current: 1489.5, percentile: 88.0, judgment: "🔴 원화 약세 — 1,490원대, 수입물가 부담 지속" };
