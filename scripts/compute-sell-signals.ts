...
// peak (고점 판단) 평가 cutoff
const DRAWDOWN_FROM_HIGH_CUTOFF = -8; // 신고가 -8% 이상 = 정점 후 후퇴
const MA200_FAR_CUTOFF = 70; // 종가가 200일선 +70%+ = 너무 멀어진 위치 (책 5범주)
const MA200_SLOPE_DOWN_THRESHOLD = -1; // 200일선 기울기 -1%+ = 하향 반전 신호
const DISTRIBUTION_WINDOW_DAYS = 5; // 최근 5일 중 distribution day 카운트
const VOLUME_DRY_CUTOFF = 0.8; // 신고가 갱신일 거래량이 60일 평균의 80% 미만 = 거래량 적은 신고가
...
interface PeakSignal {
  id: string;
  label: string;
  book_category: string; // "최후의 정점" / "약세 징후" / "지지선 붕괴" / "thesis"
  hit: boolean;
  detail: string; // raw 수치/사유
  severity: "strong" | "medium" | "weak"; // verdict 가중치
}

interface PeakEval {
  // 가격선
  ma50: number | null;
  ma200_approx: number | null; // weekCandle 40개 평균 (~200 영업일)
  ma200_slope_pct: number | null; // (최근 40주 평균 - 그 이전 40주 평균) / 그 이전 40주 평균 × 100
  ...
  high_day_volume_ratio: number | null; // 신고가 갱신일 거래량 / 60일 평균

  // distribution / climax
  distribution_days_in_5d: number;
  consecutive_down_days: number;
  largest_up_day_pct: number | null; // 최근 60일 중 최대 상승폭

  // research/monitor 통합
  monitor_alerts_count: number;
  monitor_alerts_critical: string[]; // severity='bad' 알람 제목
...
async function fetchCandles(
  code: string,
  count: number = 220,
  periodType: "dayCandle" | "weekCandle" | "monthCandle" = "dayCandle",
): Promise<PriceInfo[]> {
  const url = `https://api.stock.naver.com/chart/domestic/item/${code}?periodType=${periodType}&count=${count}`;
  const res = await fetch(url, {
    headers: { "User-Agent": UA_MOBILE, Referer: REFERER_M },
  });
  if (!res.ok) throw new Error(`네이버 캔들 실패 (${code} ${periodType}): ${res.status}`);
...
// research/monitor JSON 알람 통합
interface MonitorAlert {
  severity: "good" | "bad" | "info" | string;
  type: string;
  title: string;
  message?: string;
}
...
function loadMonitorAlerts(code: string): MonitorData | null {
  ...
  try {
    return JSON.parse(fs.readFileSync(filePath, "utf-8")) as MonitorData;
  } catch {
    return null;
  }
}
...
/**
 * peak 평가 — 책 5(최후의 정점)·6(약세 징후)·7(지지선 붕괴)·10(뉴스·실적) 통합.
 *
 * 1차 구현 핵심 신호:
 * - 50일선 위치 + 종가 vs 50일선
 * - 200일선 근사 (weekCandle 40개 평균) + 기울기 + 종가 vs 200일선
 * - 신고가 대비 -8% 이상 (정점 후 후퇴)
 * - 200일선 +70%+ 괴리 (책 5범주, 너무 멀어진 위치)
 * - distribution day count (최근 5일 중 거래량 증가 + 가격 하락)
 * - 연속 하락일 수
 * - 거래량 변화 (최근 vs 60일 평균)
 * - research/monitor 종목별 thesis 알람 통합
 */
function computePeak(
  h: Holding,
  dayCandles: PriceInfo[],
  weekCandles: PriceInfo[],
): PeakEval {
  const closes = dayCandles.map((c) => c.closePrice ?? 0).filter((v) => v > 0);
  const ma50 =
    closes.length >= 50 ? mean(closes.slice(-50)) : null;
  const lastClose = closes[closes.length - 1] ?? h.current_price;

  // 200일선 근사: weekCandle 40개 평균 ≈ 40주 = 200 영업일
  const weekCloses = weekCandles
    .map((c) => c.closePrice ?? 0)
    .filter((v) => v > 0);
  const ma200Approx =
    weekCloses.length >= 40 ? mean(weekCloses.slice(-40)) : null;

  // 200일선 기울기: 최근 40주 평균 vs 그 이전 40주 평균
  let ma200SlopePct: number | null = null;
  if (weekCloses.length >= 80) {
    const recent40 = mean(weekCloses.slice(-40));
    const prev40 = mean(weekCloses.slice(-80, -40));
    if (prev40 > 0) ma200SlopePct = ((recent40 - prev40) / prev40) * 100;
  }

  const priceVsMa50Pct = ma50 ? ((lastClose - ma50) / ma50) * 100 : null;
  const priceVsMa200Pct = ma200Approx
    ? ((lastClose - ma200Approx) / ma200Approx) * 100
    : null;

  // 신고가 (보유 시작일부터 측정) + 신고가 갱신일 거래량
  let currentHighPrice: number | null = null;
  let currentHighDate: string | null = null;
  let highDayVolume: number | null = null;
  for (const c of dayCandles) {
    if ((c.highPrice ?? 0) > (currentHighPrice ?? 0)) {
      currentHighPrice = c.highPrice!;
      currentHighDate = c.localDate ?? null;
      highDayVolume = c.accumulatedTradingVolume ?? null;
    }
  }
  // journal.holdings.high_price를 우선 사용 (이미 업데이트되어 있음)
  if (h.high_price && h.high_price > (currentHighPrice ?? 0)) {
    currentHighPrice = h.high_price;
    currentHighDate = h.high_price_date ?? currentHighDate;
  }
  const drawdownFromHighPct = currentHighPrice
    ? ((lastClose - currentHighPrice) / currentHighPrice) * 100
    : null;

  // 거래량 분석 (60일 평균)
  const volumes = dayCandles
    .map((c) => c.accumulatedTradingVolume ?? 0)
    .filter((v) => v > 0);
  const volumeAvg60 = volumes.length >= 60 ? mean(volumes.slice(-60)) : null;
  const recentVolumeAvg =
    volumes.length >= 5 ? mean(volumes.slice(-5)) : null;
  const recentVolumeRatio =
    recentVolumeAvg && volumeAvg60 ? recentVolumeAvg / volumeAvg60 : null;
  const highDayVolumeRatio =
    highDayVolume && volumeAvg60 ? highDayVolume / volumeAvg60 : null;

  // distribution day count (최근 5일 중 거래량 증가 + 가격 하락)
  let distributionDays = 0;
  const recentN = dayCandles.slice(-DISTRIBUTION_WINDOW_DAYS - 1);
  for (let i = 1; i < recentN.length; i++) {
    const prev = recentN[i - 1];
    const curr = recentN[i];
    const priceDown = (curr.closePrice ?? 0) < (prev.closePrice ?? 0);
    const volumeUp =
      (curr.accumulatedTradingVolume ?? 0) >
      (prev.accumulatedTradingVolume ?? 0);
    if (priceDown && volumeUp) distributionDays += 1;
  }

  // 연속 하락일
  let consecutiveDown = 0;
  for (let i = dayCandles.length - 1; i > 0; i--) {
    const curr = dayCandles[i].closePrice ?? 0;
    const prev = dayCandles[i - 1].closePrice ?? 0;
    if (curr < prev) consecutiveDown += 1;
    else break;
  }

  // 최대 일일 상승폭 (최근 60일)
  let largestUpDay: number | null = null;
  const recent60 = dayCandles.slice(-60);
  for (let i = 1; i < recent60.length; i++) {
    const prev = recent60[i - 1].closePrice ?? 0;
    const curr = recent60[i].closePrice ?? 0;
    if (prev > 0) {
      const pct = ((curr - prev) / prev) * 100;
      if (largestUpDay === null || pct > largestUpDay) largestUpDay = pct;
    }
  }

  // research/monitor 알람
  const monitor = loadMonitorAlerts(h.code);
  const monitorAlerts = monitor?.alerts ?? [];
  const monitorBad = monitorAlerts.filter((a) => a.severity === "bad");

  // 신호 종합
  const signals: PeakSignal[] = [];
  ...
  if (ma50 && lastClose < ma50) {
    signals.push({
      id: "below_ma50",
      label: "종가가 50일선 아래",
      book_category: "지지선 붕괴",
      hit: true,
      detail: `종가 ${lastClose.toLocaleString()} vs 50일선 ${Math.round(ma50).toLocaleString()} (${priceVsMa50Pct?.toFixed(2)}%)`,
      severity: "strong",
    });
  }

  // [지지선 붕괴] 종가 < 200일선 + 200일선 우하향
  if (
    ma200Approx &&
    lastClose < ma200Approx &&
    ma200SlopePct !== null &&
    ma200SlopePct < MA200_SLOPE_DOWN_THRESHOLD
  ) {
    signals.push({
      id: "ma200_breakdown",
      label: "200일선 이탈 + 우하향",
      book_category: "지지선 붕괴",
      hit: true,
      detail: `200일선 ${Math.round(ma200Approx).toLocaleString()} 아래 + 기울기 ${ma200SlopePct.toFixed(2)}%`,
      severity: "strong",
    });
  }

  // [최후의 정점] 200일선 +70%+ 괴리
  if (priceVsMa200Pct !== null && priceVsMa200Pct >= MA200_FAR_CUTOFF) {
    signals.push({
      id: "ma200_far",
      label: "200일선 +70%+ 괴리 (너무 멀어짐)",
      book_category: "최후의 정점",
      hit: true,
      detail: `200일선 대비 +${priceVsMa200Pct.toFixed(1)}% (책 기준 ≥ ${MA200_FAR_CUTOFF}%)`,
      severity: "strong",
    });
  }
  ...
  if (
    ...
    drawdownFromHighPct <= DRAWDOWN_FROM_HIGH_CUTOFF
  ) {
    signals.push({
      id: "drawdown_8pct",
      label: "신고가 -8% 이상 후퇴",
      book_category: "최후의 정점",
      hit: true,
      detail: `신고가 ${currentHighPrice?.toLocaleString()} (${currentHighDate}) 대비 ${drawdownFromHighPct.toFixed(2)}%`,
      severity: "medium",
    });
  }

  // [최후의 정점] distribution day 3+ in 5d
  if (distributionDays >= 3) {
    signals.push({
      id: "distribution_days",
      label: "매물 출회 신호 (최근 5일 distribution day 3+)",
      book_category: "최후의 정점",
      hit: true,
      detail: `최근 5일 중 ${distributionDays}일 거래량 증가 + 가격 하락`,
      severity: "medium",
    });
  }
  ...
  // [약세 징후] 거래량 적은 신고가
  if (
    highDayVolumeRatio !== null &&
    highDayVolumeRatio < VOLUME_DRY_CUTOFF
  ) {
    signals.push({
      id: "low_volume_high",
      label: "거래량 적은 신고가",
      book_category: "약세 징후",
      hit: true,
      detail: `신고가일(${currentHighDate}) 거래량이 60일 평균의 ${(highDayVolumeRatio * 100).toFixed(0)}% (책 기준 < 80%)`,
      severity: "weak",
    });
  }
  ...
  if (consecutiveDown >= 4) {
    signals.push({
      ...
      book_category: "약세 징후",
      hit: true,
      detail: `현재 ${consecutiveDown}일 연속 하락`,
      severity: "weak",
    });
  }

  // [thesis] research/monitor critical alert
  if (monitorBad.length > 0) {
    signals.push({
      id: "monitor_thesis_break",
      label: "종목별 thesis 알람",
      book_category: "thesis",
      hit: true,
      detail: `${monitorBad.length}건: ${monitorBad
        .slice(0, 3)
        .map((a) => a.title)
        .join(" / ")}`,
      severity: "medium",
  ...
  return {
    ...
    ma200_approx: ma200Approx != null ? Math.round(ma200Approx) : null,
    ma200_slope_pct:
      ma200SlopePct != null ? parseFloat(ma200SlopePct.toFixed(2)) : null,
    price_vs_ma50_pct:
      priceVsMa50Pct != null ? parseFloat(priceVsMa50Pct.toFixed(2)) : null,
    price_vs_ma200_pct:
      priceVsMa200Pct != null ? parseFloat(priceVsMa200Pct.toFixed(2)) : null,
    current_high_price: currentHighPrice,
    current_high_date: currentHighDate
      ? currentHighDate.includes("-")
        ? currentHighDate
        : `${currentHighDate.slice(0, 4)}-${currentHighDate.slice(4, 6)}-${currentHighDate.slice(6, 8)}`
      : null,
    drawdown_from_high_pct:
      drawdownFromHighPct != null
        ? parseFloat(drawdownFromHighPct.toFixed(2))
        : null,
    recent_volume_avg: recentVolumeAvg != null ? Math.round(recentVolumeAvg) : null,
    volume_avg_60d: volumeAvg60 != null ? Math.round(volumeAvg60) : null,
    recent_volume_ratio:
      recentVolumeRatio != null
        ? parseFloat(recentVolumeRatio.toFixed(2))
        : null,
    high_day_volume_ratio:
      highDayVolumeRatio != null
        ? parseFloat(highDayVolumeRatio.toFixed(2))
        : null,
    distribution_days_in_5d: distributionDays,
    consecutive_down_days: consecutiveDown,
    largest_up_day_pct:
      largestUpDay != null ? parseFloat(largestUpDay.toFixed(2)) : null,
    monitor_alerts_count: monitorAlerts.length,
    monitor_alerts_critical: monitorBad.map((a) => a.title),
    signals,
    hit_count: signals.length,
  };
}

function determinePeakVerdict(peak: PeakEval): Verdict {
  const reasons: string[] = [];
  const strongCount = peak.signals.filter((s) => s.severity === "strong").length;
  const mediumCount = peak.signals.filter((s) => s.severity === "medium").length;
  const weakCount = peak.signals.filter((s) => s.severity === "weak").length;

  let verdict: Verdict["verdict"] = "HOLD";

  if (strongCount >= 2 || (strongCount >= 1 && mediumCount >= 2)) {
    verdict = "SELL";
    reasons.push(
      `고점·지지선 신호 다수 hit (강 ${strongCount} / 중 ${mediumCount}) — 본격 정점 + 지지선 붕괴`,
    );
  } else if (strongCount >= 1) {
    verdict = "TRIM";
    reasons.push(
      `지지선 또는 200일선 신호 hit (강 ${strongCount}) — 분할 매도 검토`,
    );
  } else if (mediumCount >= 2 || (mediumCount >= 1 && weakCount >= 2)) {
    verdict = "WATCH";
    reasons.push(
      `약세 신호 누적 (중 ${mediumCount} / 약 ${weakCount}) — 추세 확인 필요`,
    );
  } else if (mediumCount >= 1 || weakCount >= 1) {
    verdict = "WATCH";
    reasons.push(`초기 약세 신호 1건 — 모니터링`);
  }

  for (const s of peak.signals) {
    reasons.push(`[${s.book_category}] ${s.label}: ${s.detail}`);
  }

  if (verdict === "HOLD" && peak.signals.length === 0) {
    reasons.push("정점·약세·지지선·thesis 신호 모두 미달 — 추세 유지");
  }
...
async function main() {
  ...
  for (const h of targets) {
    ...
    // 1) 네이버 캔들 (day 110 + week 110 병행)
    const [candles, weekCandles] = await Promise.all([
      fetchCandles(h.code, 110, "dayCandle"),
      fetchCandles(h.code, 110, "weekCandle"),
    ]);
    if (candles.length === 0) {
    ...
    // 5) peak 평가 (책 5·6·7·10 범주)
    const peak = computePeak(h, candles, weekCandles);
    const peakVerdict = determinePeakVerdict(peak);
    ...
    console.log(
      `  50일선 ${ma50 != null ? Math.round(ma50).toLocaleString() : "-"} / 200일선(weekCandle 40개 평균) ${ma200 != null ? Math.round(ma200).toLocaleString() : "-"}`,
    );
    console.log(
      `  [strategy] verdict: ${strategyVerdict.verdict} — ${strategyVerdict.reasons.join("; ")}`,
    );
    console.log(
      `  [peak] 신고가 ${peak.current_high_price?.toLocaleString() ?? "-"} (${peak.current_high_date ?? "-"}) 대비 ${peak.drawdown_from_high_pct?.toFixed(2) ?? "-"}% / 200일선 괴리 ${peak.price_vs_ma200_pct?.toFixed(2) ?? "-"}% / 거래량 60일 대비 ${peak.recent_volume_ratio?.toFixed(2) ?? "-"}배 / distribution ${peak.distribution_days_in_5d}일 / 연속하락 ${peak.consecutive_down_days}일 / monitor 알람 ${peak.monitor_alerts_count}건`,
    );
    console.log(
      `  [peak] 신호 ${peak.hit_count}건: ${peak.signals.map((s) => `${s.id}(${s.severity})`).join(", ") || "없음"}`,
    );
    console.log(
      `  [peak] verdict: ${peakVerdict.verdict} — ${peakVerdict.reasons[0] ?? "-"}`,
    );
  }
