...
발상은 사용자의 "잘 나갔던 주식이 또 잘 나간다" — 네 항목이 모두 '이미 얼마나 올랐나'를 본다.
방법충실 돌파 백테스트(2022-2026, 트렌드템플릿+검출기 피벗돌파 진입, -8%손절/트레일)로 검증.
검증 결과: 점수 4+ = 6개월 내 '더블(+100%)' 도달률 36% vs 0~1점 15% (전체 평균 28%).

★적용 범위 (26-08-22 확정) — **+20%/-10% 단기 매매의 매수 필터로 쓰지 말 것.**
이 점수가 맞히는 건 '6개월 대박'이지 '+20% 도달'이 아니다. 백테스트 614건에서
+20% 도달률은 1점 46.3% / 3점 38.1% / 5점 34.6% 으로 역단조(상관 -0.044)였고,
같은 스캔일 비교로는 차이가 없다(25승 32패 22무) → "낮은 점수가 낫다"도 주장 금지.
확정된 건 "거를 이유가 없다" 하나뿐. 실사례: 3점 미만을 걸러 현대해상(1점) +24.5% 미매수.
또한 위 '36% vs 15%'는 26-07-10 산출 이후 재검증된 적이 없다(같은날 비교·종목 순열 미적용).
장기 보유 선별로 용도를 바꾸면 그 숫자부터 다시 검증할 것. 메모리: superperf-score-scope.
점수 요인(최대 6점):
