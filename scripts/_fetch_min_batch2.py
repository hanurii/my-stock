...
def to3(bars):
    ...
    return out
codes=json.load(open('scripts/_tmp_codes2.json',encoding='utf-8'))
res={}
for nm,code in codes.items():
    b=fetch(code); b3=to3(b); res[nm]={'code':code,'bars':b3}
    print(f"{code} {nm:14s} 3min={len(b3):3d}", flush=True)
json.dump(res, open('scripts/_min3_20260624_b2.json','w',encoding='utf-8'), ensure_ascii=False)
print('saved')
