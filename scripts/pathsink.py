...
사이드카 취급
------------
🚨 `<out>.paths.jsonl` 은 **중간 산출**이다. **`.gitignore` 에 `*.paths.jsonl` 로 걸어 뒀고,
`merge_paths()` 가 병합 뒤 «지운다».** 남아 있으면 병합이 안 끝난 것이다.

⚠️ **2026-08-24 사고**: 넘겨받는 `out_path` 가 «경로»가 아니라 **불리언 `True`** 여서
저장소 루트에 `True.paths.jsonl`(50MB) 이 생겼다. 변수 이름은 `EMIT_PATHS` 인데
값이 「낼까 말까」의 불리언이었다. **옛 판은 그 값을 «있나 없나»로만 써서 안 드러났고,
이 모듈이 그 값으로 «파일 이름을 만드는 순간» 터졌다.**
★ **이름이 값을 잘못 말하고 있으면, 그 값을 «새로운 방식으로 쓰는 순간» 터진다.**
"""
