...
# ── ② strategy_params.py — 상수 이름을 손으로 나열하지 않는다. ───────────
# _harvest_constants() 가 모듈 최상위의 «모든 대문자 상수»를 스스로 찾는다.
# 기준 커밋이 e8cd65ae 가 «아니다» — 사용자가 2026-09-02 에 TARGET_PCT 를
# 20.0에서 30.0 으로 스스로 바꿨다(41db459d). 그 결정 «자체»는 지켜야 할
# 값이지 잡아낼 결함이 아니다.
CONST_TARGETS = [
    ("scripts/canslim_lib/strategy_params.py", "41db459d",
     "사용자 결정 2026-09-02(익절 +20에서 +30, 커밋 41db459d). 그 뒤로 안 바뀌어야 한다."),
