...
export const CONFIGS: MonitorConfig[] = [
  // ───── 아바코 (083930) ─────
  {
    ...
    // news_keywords 매도 신호 전용 정비 (2026-04-26): "중국 OLED 보조금" 양면 →
    // "보조금 축소"로 부정 명시. BOE 투자 둔화·중국 규제 강화는 그대로.
    news_keywords: [
      "중국 디스플레이 장비 수출 규제",
      "중국 디스플레이 장비 규제 강화",
      "중국 OLED 보조금 축소",
  ...
  // 뉴스: 삼성 HBM 점유율 추격 / DRAM 가격 / 엔비디아 Capex
  {
    ...
    // news_keywords 매도 신호 전용 정비 (2026-04-26): "엔비디아 Capex 가이던스"는
    // 양면(중립)이라 "Capex 축소"로 부정 명시. 나머지는 SK하이닉스 입장에서 모두 부정.
    news_keywords: [
      // 삼성 HBM 추격 (SK하이닉스 점유율 잠식)
      "삼성 HBM3E 엔비디아 점유율",
      "삼성 HBM4 엔비디아 공급",
      "삼성 HBM4 엔비디아 계약",
      "삼성 HBM4 엔비디아 퀄 통과",
  ...
  // 5. 뉴스: 파운드리 분사·매각, HBM4 엔비디아 공급 성공/실패
  {
    ...
    // news_keywords 매도 신호 전용 정비 (2026-04-26): "엔비디아 공급/계약"·
    // "자사주 소각"은 호재. "엔비디아 탈락"·"양산 지연"·"자사주 철회"로 교체.
    news_keywords: [
      // HBM 사업 부정 신호 (엔비디아 퀄 실패·삼성 추격 차단)
      "삼성 HBM4 엔비디아 탈락",
      "삼성 HBM4 퀄 실패",
      "삼성 HBM4 양산 지연",
      "삼성 HBM3E 엔비디아 탈락",
  ...
  // (주가 +50% 급등 + 외국인 변화는 평단 참조 복잡 — 생략)
  {
    ...
    // news_keywords 매도 신호 전용 정비 (2026-04-26): "중동 수주"·"원전 플랜트"·
    // "방산 수주" 호재 키워드 제거. SNT홀딩스 EB는 부정 명시라 유지.
    news_keywords: [
      // SNT홀딩스 EB·블록딜 (구조적 오버행)
      "SNT홀딩스 교환사채",
      "SNT홀딩스 EB",
      "SNT홀딩스 블록딜",
      "SNT홀딩스 매각",
  ...
  // + 뉴스: 원전/발전 수주, 주식담보제공, 자금대여
  {
    ...
    // news_keywords 매도 신호 전용 정비 (2026-04-26): "원전 수주"·"HRSG" 호재
    // 키워드 제거. 오너 매도·자금 흐름 부정 신호로 정비.
    news_keywords: [
      // 오너 일가 매도·블록딜 (지배구조 부정)
      "우종인 주식담보",
      "우종인 매도",
      "박은미 매도",
      "박은미 블록딜",
      "비에이치아이 블록딜",
      // 비상장 가족회사 자금 흐름 (이익 유출)
      "비에이치아이건설 자금",
      "비에이치아이 금전대여",
      "비에이치아이 자금대여",
      // 자사 부정 신호
  ...
  // 6. 함영주 회장 사법 리스크 — 뉴스 키워드
  {
    ...
    // news_keywords 매도 신호 전용 정비 (2026-04-26): 호재 매칭 키워드 제거.
    // 일반 토큰("분기배당"·"자사주"·"주주환원")은 회사 발표 호재까지 잡으므로
    // 부정 명시 phrase 또는 인물명 + 부정 동사 조합으로만 작성.
    news_keywords: [
      // 자사 펀더멘털 부정 신호
      "하나금융 충당금 적립",
      "하나금융 부실채권",
  ...
  // 6. 국민연금 지분 처분 — bank_corp_disclosures (KB국민은행 5% 변동 보고) + 뉴스 키워드
  {
    ...
    // news_keywords 매도 신호 전용 정비 (2026-04-26): 자사주 소각·분기배당
    // 호재 매칭 키워드 제거. 부정 명시 phrase + 인물명 부정 동사 조합으로 정비.
    news_keywords: [
      // 자사 펀더멘털 부정 신호
      "KB금융 NIM 압축",
      "KB금융 NIM 하락",
  ...
  // + 뉴스: 김기홍 회장·사법 리스크·환원 후퇴
  {
    ...
    // news_keywords는 매도 신호 전용 — 매칭 0~5건이 정상.
    // 정량 메트릭 11개가 주 신호이고, 뉴스는 매크로·돌발 리스크 보강.
    // 일반 토큰("배당"·"환원"·"자사주"만 들어간 표현)은 구글 RSS가 토큰 매칭으로
    // 호재까지 잡아오므로 phrase에 부정 형용사 명시(부실채권·충당금 적립·검찰·구속 등)
    // 또는 인물명 + 부정어 조합으로만 작성.
    news_keywords: [
      // 자사 펀더멘털 부정 신호 (한국 뉴스에 실제 등장 패턴)
      "JB금융 충당금 적립",
      "JB금융 부실채권",
      "JB금융 NIM 하락",
      "JB금융 건전성 악화",
      // 자회사 PF·리스크 (지방은행 PF는 가장 큰 단일 리스크)
      "전북은행 부동산 PF",
      "광주은행 부동산 PF",
      "전북은행 충당금",
      "광주은행 충당금",
  ...
  // (전량매도 ③ 지배구조 개편 시 우선주 차별은 정성 판단 영역 — 뉴스 키워드만 추적)
  {
    ...
    // news_keywords 매도 신호 전용 정비 (2026-04-26): "현대차 자사주 소각"
    // 호재 매칭 제거. "멕시코 공장" 양면 → "멕시코 공장 폐쇄/적자"로 부정 명시.
    news_keywords: [
      // 관세·매크로 부정 (트럼프 관세는 시점에 따라 부과 자체가 부정 신호)
      "트럼프 자동차 관세",
      "현대차 미국 관세",
      "현대차 관세 부담",
      "현대차 미국 판매 감소",
