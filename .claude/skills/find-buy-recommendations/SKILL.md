...
뽑는다. 점수 정의: `scripts/canslim_lib/superperf.py`.
발상은 사용자의 **"잘 나갔던 주식이 또 잘 나간다"** 를 점수화한 것으로, 네 항목이 모두
"이미 얼마나 올랐나"를 본다(직전 상승폭·RS·RS선 신고가·RS선 선행).

> **⚠️ 점수의 적용 범위 — 매수 필터로 쓰지 말 것 (26-08-22 확정).**
> 이 점수가 맞히는 건 **6개월 내 더블(+100%)** 이지 **+20% 단기 목표**가 아니다.
> 백테스트 614건(2025-11~2026-08)에서 +20% 도달률은 1점 46.3% · 2점 41.4% · 3점 38.1% ·
> 4점 35.7% · 5점 34.6%로 **역단조**였고(상관 -0.044), **같은 스캔일 비교로는 차이 없음**
> (25승 32패 22무, 중앙 0.0%p) → "낮은 점수가 낫다"도 주장 금지. 확정된 건
> **"+20% 매매에서 점수로 거를 이유가 없다"** 하나뿐이다. 실제 손실 사례: 3점 미만을
> 걸러 현대해상(1점, 8/7 VCP 진입임박·리스트 5/9위)의 **+24.5%** 를 놓쳤다.
> 또한 **"4점↑ 6개월 더블 36% vs 0~1점 15%"는 26-07-10 산출 이후 재검증된 적 없다**
> — 같은날 비교·종목 순열 검정 미적용. 장기 보유 선별로 방향을 틀면 이 숫자부터 재검증할 것.
> 근거: 메모리 `superperf-score-scope`.
