...
- **분할 매수(lot) 병합**(사용자 확정 26-08-08): 같은 종목 여러 매수 항목은 점검
  단계에서 한 항목으로 합친다 — 수량 합산·매수가 혼합단가, 판정 기준(매수일·피벗·
  셋업·손절%)은 **가장 최근 lot**(추가 매수 = 셋업 재설정으로 간주해 돌파 품질·
  경과일을 최근 매수 기준으로 측정). holdings 파일 자체는 계속 lot 단위 기록,
  병합 내역은 산출의 `lots` 필드에 보존.
  없거나 비면 빈 결과로 정상 종료.
