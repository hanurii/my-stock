...
- `--universe all` : 트렌드 통과 종목 대신 **현재 상장 KOSPI+KOSDAQ 전체**를 스캔(탐색용).
  - 트렌드 템플레이트/RS 필터 **없음** — 패턴 자체만 본다(추세·상대강도 미반영).
  - 유니버스는 `pykrx_universe.fetch_universe_with_cap("all")`(트렌드 스크리너와 동일)
    이라 **KONEX·상폐·ETF·ETN·스팩·리츠·우선주·거래정지는 제외**되고, 그중 OHLCV 캐시
    있는 종목만 평가한다(전체 OHLCV 캐시 glob 아님 — 캐시엔 KONEX·상폐가 섞여 있음).
  - 산출: `public/data/sepa-power-play-universe.json`(+`caveat` 명시). FDR 상장목록
    네트워크 조회가 필요(트렌드 스크리너와 동일 의존).
...
## 안 하는 것
- VCP 베이스 탐지(그건 find-vcp) · 공유 파일 갱신 · 수급 신호 · 자동 commit.
- 기본은 트렌드 통과 종목만 스캔. 전 종목 탐색은 `--universe all` 로 **명시 opt-in**할 때만
  (그래도 KONEX·상폐·ETF는 제외, 추세/RS 필터는 없는 탐색용).
