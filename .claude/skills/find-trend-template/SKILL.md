...
- `candidates[].all_pass == true` 인 종목이 SEPA 다음 단계 입력. 추가로
  **관문 완화(gate_near)는 26-08-21부터 전면 중단** — 미너비니 8조건 전부 필수다
  (`GATE_NEAR_ENABLED = False`). 되살리려면 그 스위치만 True 로 바꾼다.
  완화 변천: 26-08-11 도입(①150·200MA -10%·⑤50MA -15%·⑦52주고가 -40%)
  → 26-08-12 ⑦ 필수 환원 → 26-08-17 ⑦ -35% 재완화
  → **26-08-21 ⑦ 필수 환원 + ①⑤도 중단(사용자 결정 "완화도 끕시다")**.
  중단 근거: ⑦ 완화로 들어온 종목이 실전 7건 0승(청산 4건 -413만원)이고,
  8/18 손절 6종목 전원이 52주고가 -12~-35% 딥베이스였던 반면 승자(DN오토·골프존·
  아스플로)는 전부 -4% 이내. 사용자 우선순위 "좋은 주식을 놓치는 것보다 손실을
  막는 것". ①⑤ 자체는 실패 근거가 없었으나(8월 200일선 아래 매수 0건) 기준을
  일관되게 적용. 감수하는 트레이드오프: 한양이엔지형(이평선 바로 밑 코일)은
  돌파 전날 관문에 걸려 미포착.
  형제 검출기·매수추천은 `gate_near` 필드를 계속 읽지만 항상 false 다.
