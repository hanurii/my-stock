---
name: find-vcp-us
description: 미국 SEPA 2단계. 1단계(find-trend-template-us) 통과 종목의 일봉에서 미너비니 VCP(변동성 수축 패턴)를 탐지하고 피벗과 진입상태(breakout/actionable/forming/failed)를 산출해 sepa-us-vcp-candidates.json 에 저장한다. 검출기 코드와 값은 한국과 같은 모듈을 그대로 쓴다. 한국 후보 파일은 건드리지 않는다. 사용자가 "/find-vcp-us", "미국 VCP", "미국 2단계" 등을 요청할 때 사용.
---
...
2. `python scripts/screen_detectors_us.py --kind vcp`
...
## 산출
`public/data/sepa-us-vcp-candidates.json`
...
- `params` 에 그날 쓰인 값이 그대로 실린다. 이 값은 `canslim_lib/vcp.py` 의
  `DEFAULT_PARAMS` 이고 27.4년 백테스트가 돈 값이다.

## 되돌리지 말 것
- 검출기에 `params` 를 넘기지 않는다. 넘기면 `verify_frozen_params.py` 가 얼린 값을
  우회하게 된다. 값을 바꾸고 싶으면 모듈의 `DEFAULT_PARAMS` 를 고치고 검산기를 통과시켜라.
- 계열은 1단계 산출의 `asof` 까지 잘라서 검출기에 넘긴다. 안 자르면 1단계를
  `--asof` 로 과거에 돌린 날 2단계가 1단계가 못 본 봉을 보게 된다(룩어헤드).

## 자료
어느 시세 파일을 읽었는지 첫 줄에 찍힌다. 갱신했는데 `뼈대만` 으로 나오면
`/update-data-us` 의 꼬리 단계가 안 돈 것이다.

## 안 하는 것
전 종목 스캔(1단계 통과분만) · 한국 파일 갱신 · 자동 커밋.
