...
7. **매수 추천 리스트 — `find-buy-recommendations` 스킬 호출.** 검출된 후보(VCP/
   파워플레이/3C) 중 초수익 잠재력 점수(직전상승폭·RS·RS선)를 매겨 점수순
   `sepa-buy-recommendations.json` 생성(`/stocks/sepa` '매수 추천 리스트' 섹션이 읽음).
   - **반드시 3단계(형제 패턴)가 전부 끝난 뒤 실행** — 후보 파일을 읽는다.
   - **비차단**: 실패(예: FDR 지수 수집 오류)해도 파이프라인 실패로 보지 않고 오류
     한 줄만 보고 후 넘어간다(그 경우 아래 커밋에서 `sepa-buy-recommendations.json`
     은 빼고 나머지만 커밋).
7.5 **주도 태깅(수출 + 섹터) — `python scripts/tag_export_leaders.py` 와
   `python scripts/tag_leading_sectors.py`.** (오케스트레이터 전용 마무리 스텝 —
   후보 4파일+매수추천에 등장한 종목을 ① 수출 주도 품목(설정:
   `export-leading-config.json`, 키워드+오버라이드)으로 분류해 `sepa-export-tags.json`,
   ② 주도 섹터 ①~⑧ + ⑨백화점 동행 소비주(설정: `leading-sectors-config.json`, 큐레이션 코드→섹터 매핑,
   사용자 직접 관리)로 분류해 `sepa-leading-sectors.json` 산출. `/stocks/sepa` 후보
   테이블·매수 추천의 🚢·①~⑨ 배지가 읽는다. 표시 전용 — 정렬·점수·보유 점검 무관여.
   같은 라벨 이중 표시는 페이지가 섹터 배지 우선으로 자동 정리(예: ②반도체+🚢반도체 → ②만).)
   - 후보·매수추천 파일을 읽으므로 **3단계와 7단계 뒤에** 실행.
   - **비차단**: 실패해도 파이프라인 실패 아님 — 오류 한 줄 보고 후 진행
     (그 경우 아래 커밋에서 해당 산출 파일은 빼고 나머지만 커밋).
...
7.9 **NXT 프리마켓 거래량 분모 — `python -X utf8 scripts/build_premarket_vol.py --days 20`.**
   (오케스트레이터 전용 마무리 스텝 — 감시 대상 종목의 NXT 프리마켓 08:00~08:50
   «분 단위 누적» 거래량을 20거래일치 모아 `.cache/premarket/premarket-vol-history.json`
   에 쌓는다. 장중 알림 도구(`scripts/pivot_alert.py`)가 아침에 「지금 거래량이 평소
   이 시각의 몇 배인가」를 재는 **분모**로 쓴다.)
   - **반드시 3단계 뒤에** 실행 — 감시 대상을 후보 파일에서 뽑기 때문이다.
     여기서 돌려야 **다음 날 아침 감시 목록 기준**으로 분모가 준비된다.
     아침에 돌리면 늦다: 감시 목록은 전날 저녁 이 파이프라인이 정하고, 하루 사이
     7종목쯤 새로 들어온다(26-09-21 실측 — 들어옴 7 · 빠짐 9).
   - 이미 받은 날짜는 다시 안 받는다(증분). 감시 57종목이면 하루 ~7초,
     처음 20일치를 채울 때만 ~2.4분.
   - **커밋 안 함** — 로컬 도구 전용 캐시다(398KB, 날마다 늘어남). `.cache/` 아래라
     gitignore 대상이고 웹 페이지는 안 읽는다.
   - **비차단**: 실패해도 파이프라인 실패 아님 — 오류 한 줄 보고 후 진행.
8. **통합 요약 보고** — 표 하나로:
   ...
   - 매수 추천: 초수익 점수 상위 N종목(점수·종목명 — screen_buy_recommendations 콘솔 그대로)
9. **자동 commit + push** — 커밋 대상은 아래 **결과 파일만**. 다른 변경
   ...
   - `public/data/sepa-buy-recommendations.json` (7단계 매수 추천 — /stocks/sepa
...
10. **캐시 백업(비차단) — `python scripts/backup_cache.py`.** 파이프라인 성공·
...
- 조건주문표(`scripts/make_order_sheet.py`) — 정기 단계 아님. 사용자가 상황별
  직접 판단을 선호해 페이지 섹션·정기 산출에서 제외(26-08-12). HTS 자동감시주문
  예약이 필요할 때만 온디맨드 실행(감시가·상한·수량 콘솔 출력).
