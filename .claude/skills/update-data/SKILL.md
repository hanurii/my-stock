...
- `--update` : pdata에서 **새 영업일만** fetch(보통 1일)해 종목별 시계열에 추가.
  이미 받은 날은 캐시 hit.
- `--fill-fdr` : pdata가 아직 못 준 당일/전일을 FDR(KRX 기반)로 보충.
  - 🚨 **FDR 종가는 «그날 저녁까지만» 정규장 종가다.** 2026-09-14 애프터마켓
    개장 뒤로 밤사이 통합 종가로 정산된다(26-09-22 실측: 09-21 어긋남 84.2%,
    당일 09-22 는 0.0%). 그래서 이 옵션은 **당일분 메우기 전용**이다. pdata 가
    그 날을 주기 시작하면 `update_to_latest` 가 시계열을 다시 만들며 정규장
    종가로 덮어쓴다. 헌법 §2.3 의 출처별 표를 볼 것.
...
```
python -X utf8 scripts/verify_close_gate.py
```

- **exit 0 통과 · 1 실패 · 2 못 가림.** 1 이면 **뒤 단계로 넘어가지 않는다** —
  종가가 틀리면 피벗·RS·200일선이 전부 틀린다.
- 자는 **채우기 사슬(pdata·FDR) «밖»**에 있다. 거래소가 스스로 내는 기준가로
  잰다: 기준가(D) = 종가(D) − prdy_vrss(D) 이고 그건 정의상 전일 정규장 종가다.
- 🚨 **왜 사슬 밖이어야 하나**: 26-09-16 에 pdata 와 FDR «둘끼리만» 맞대 보고
  틀린 쪽을 골랐다. 두 출처가 다르다는 사실은 어느 쪽이 맞는지를 **못 말한다**.
  그 고침이 09-14~09-21 캐시 종가의 60~66% 를 어긋나게 했고 엿새를 못 잡았다.
- 못 가림(2)은 자가 안 선 것이다 — 기준가 연쇄가 깨졌거나 표본이 비었다.
  **통과로 읽지 않는다.**
