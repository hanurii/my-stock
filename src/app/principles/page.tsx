    ...
    principles: [
      {
        ...
        how: "배당 25% 안 구성: 한국 고배당 ETF 15% + 우량 배당주 2~3종목 10%. 성장주 25% 안 구성: 기둥(개별 종목 4~5개) + 위성(사이클 수혜 ETF 1~2개). 목표 시점 2027/Q1, 분기당 7%p 분할 매도로 현재 51% 성장주를 25%로 이행.",
      },
      {
        number: "1.2",
  ...
  },
  {
    ...
    principles: [
      ...
      },
      {
        ...
        how: "매도 시점에 다음을 캘린더·증권 앱 알림에 등록. 매도가 -10%(1/3) / -20%(1/3) / -30%(1/3) 분할 매수. 마지막 1/3 안전장치: 다음 셋 중 가장 먼저 발생 — (가) 매도가 -30% 도달 (나) 매도가 -20% 도달 후 저점 +10% 반등 (다) 매도가 -20% 도달 후 3개월 경과.",
      },
      {
        number: "3.3",
        rule: "코어 코스피200 ETF 매도 금지",
        why: "코어는 FOMO 안전판이다. 코어를 흔들면 안전판이 무너진다. 단순 사이클 다운으로 -25% 빠져도 V자 반등 가능성이 있어 보유가 답.",
        how: "분기 리밸런싱(50% 비중 유지)을 위한 매도 외에는 매도 금지. 핫 섹터 진입은 성장주 영역 25% 안에서만. 다음 두 예외만 매도 가능 — (a) 슈퍼사이클(코스피·반도체 12개월 +50% 또는 반도체 ETF 코스피 대비 +30%p 아웃퍼폼(특정 주식의 상승률이 시장 평균보다 살짝 더 클 것이라고 예측하는 것)) + 사이클 다운 신호(코스피 -10% + 반도체 60일 이평 하향 + 외인 5거래일 순매도) 동시 충족. (b) 거시 위기 트리거 발동(룰 3.6).",
      },
      {
        number: "3.4",
        rule: "배당주 매도 금지",
        why: "배당주는 앵커 역할(매도 충동 억제). \"팔지 않고 오래 보게 되는 명분\"이 본인 평온의 진짜 트리거 중 하나. 자주 매매하면 그 효과 사라짐.",
        how: "분기 리밸런싱 외 매도 금지. 예외: 펀더멘털 훼손(분기 영업이익 전년 동기 대비 -40% 이상 또는 배당 삭감 발표). 거시 위기 트리거 발동 시 함께 매도.",
      },
      {
        number: "3.5",
        rule: "외인 매도 자동 보유 룰 (펀더멘털 무훼손 시)",
        why: "이상적 자기(시나리오 답변에서 침착)와 실시간 행동(외인 매도 보면 흔들림)이 어긋남을 본인 자백. 의지가 아니라 룰로 통제해야 함.",
        how: "매수 시점에 \"이런 일이 일어나면 내 가설이 깨진 것\"이라는 펀더멘털 훼손 기준 미리 정의(예: 분기 영업이익 전년 동기 대비 -20% 이하 + 컨센 미달). 그 기준이 발동 안 한 상태에서 외인 순매도 + 가격 -5% 빠지면 다시 분석하지 않고 그냥 보유.",
      },
      {
        number: "3.6",
  ...
  },
  {
    ...
    principles: [
      ...
      },
      {
        ...
        how:
          "친구·커뮤니티·유튜버 수익 자랑을 보면 매주 일요일 자동 리포트(연초 대비 누적 수익 + 누적 배당 + 코스피 대비 상대 성과)를 다시 본다. 전략 변경은 분기 단위로만.",
      },
      {
        number: "4.3",
    ...
    principles: [
      {
        ...
        how:
          "5섹션: ① 주간 변동 ② 연초 대비 누적 수익 ③ 연초 대비 누적 배당 ④ 펀더멘털 진척(주요 보유 종목 매출·영업이익 전년 동기 대비) ⑤ 코스피 대비 상대 성과. my-stock에서 자동 생성.",
      },
      {
        number: "5.2",
        rule: "분기 자기검증 — 성장주 vs 코스피 ETF 비교",
        why:
          "본인 종목 선택 능력이 진짜 시장 평균보다 나은지 객관적 데이터로 확인. (d) 자백 객관 검증. 계속 못 이긴다는 게 데이터로 드러나면 비중을 강제로 줄여 손해 키우는 걸 막기 위함.",
        how:
          "매 분기말 성장주 영역 누적 수익률 vs 같은 기간 코스피200 ETF 비교 기록. 4분기(1년) 연속으로 코스피 ETF보다 -5%p 이상 지면 성장주 비중을 자동 -5%p 축소. 의지가 아니라 룰로.",
      },
      {
        number: "5.3",
...
const heldItems = [
  ...
  },
  {
    ...
    why: "정부의 코리아 디스카운트 개선 정책 + 한국 시장 슈퍼사이클 모멘텀에 베팅한 결정. 정부 정책에 의존한다는 약점은 객관적 진입 기준(환율·박스권·정책 변화)을 미리 정해두어 보완.",
    triggers: [
      "한국 시장 지지부진 — 코스피 6개월간 ±5% 박스권 횡보",
      "달러 환율 저렴 — 원달러 환율 1,200원 이하",
      "정치 변수 변화 — 코리아 디스카운트 개선 정책 후퇴 또는 정권 교체",
    ],
    next: "위 셋 중 둘 충족 시 미국 시장 진입 검토 시작. 진입 규모: 코어 50% 안에서 미국 S&P500 ETF 10~20%p, 배당 25% 안에서 미국 배당 ETF 5~10%p, 성장주 25% 안에서 미국 저평가 성장주 5~10%p. 6~12개월 분할 진입.",
  },
];
...
const rejectedItems = [
  {
    ...
    alternative: "정보 공유는 OK. 주간 리포트와 헌장 룰북은 같이 보기 — 와이프가 \"우리 자산이 잘 운용되고 있다\"는 안심을 얻을 수 있는 수준. 의사결정 참여는 부담시키지 않음. 와이프 통제 자리는 자동화 도구(자동 매수·캘린더 알림·24시간 쿨다운·매수 전 체크리스트 강제)가 대체.",
  },
];
