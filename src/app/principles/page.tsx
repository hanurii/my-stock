...
const sections: PrincipleSection[] = [
  {
    ...
    principles: [
      {
        number: "1.1",
        rule: "코스피200 ETF 50% + 배당 25% + 성장주 25%",
        why: "FOMO 회피 → 시장 베타 추종이 우선 = 코스피200 ETF 50% 코어. 앵커(매도 충동을 억제하는 역할) + 잃지 않는 투자 + 매일 천천히 우상향 자산군 = 배당 25%. 종목 선택 능력에 대한 자기 자백을 인정하되 격리 통제 = 성장주(저평가 성장주) 25%.",
        how: "배당 25% 안 구성: 한국 고배당 ETF 15% + 우량 배당주 2~3종목 10%.\n성장주 25% 안 구성: 기둥(개별 종목 4~5개) + 위성(사이클 수혜 ETF 1~2개).\n목표 시점 2027/Q1, 분기당 7%p 분할 매도로 현재 51% 성장주를 25%로 이행.",
      },
      {
        number: "1.2",
        ...
        how: "월 250만 원 × 6개월 = 1,500만 원을 CMA/MMF/파킹통장으로 분리.\n현재 51% 성장주 → 25% 이행과 함께 진행. 한 분기에 500만 원씩 3분기 분할로 매도, 펀더멘털 약한 종목부터 정리.",
      },
      {
        number: "1.3",
        rule: "성장주 영역 종목당 상한 6%, 하한 1% (전체 자산 대비)",
        why: "한 종목이 크게 오르면 비중이 자연스럽게 커진다. 상한 없으면 단일 종목이 성장주 영역의 60%를 먹어버리는 표류 패턴이 4~5개월 동안 검증됨. \"이 종목은 진짜 좋다\"는 본능을 룰로 통제해야 함.",
        how: "분기말 6% 초과 시 절삭, 1% 미만 자투리 종목 정리.\n6% 상한이면 단일 종목 -50% 시 전체 자산 -3% 충격으로 제한.",
      },
      {
        number: "1.4",
        rule: "단일 섹터 비중 25% 이하",
        why: "섹터 사이클은 같이 움직인다. 반도체 한 섹터에 25%가 묶이면 사이클 전환기에 포트폴리오 전체가 흔들림. 코스피200 ETF에는 이미 반도체 25~35%가 자동 포함되어 있다는 점도 고려.",
        how: "메가캡·성장주 스크리닝·바이오 워치리스트 점검 시 섹터 합산 비중 항상 확인.\n코스피200 ETF의 반도체 노출까지 합산해서 계산.",
      },
      {
        number: "1.5",
        rule: "기둥 + 위성 모델 — 성장주 5~7개로 분산 + 통제감 균형",
        why: "분산형(10개+)은 분석 시간 부담 + 단일 종목 영향력 약함. 집중형(2~3개)은 \"내가 안 산 종목\"이 오를 때 FOMO 트리거. 본인의 FOMO 회피를 성장주 영역 안에서도 일관되게 적용한 구조.",
        how: "기둥(개별 종목 4~5개) — 본인이 깊게 분석하고 확신 있는 종목.\n위성(사이클 수혜 ETF 1~2개) — 본인이 좋게 본 종목들이 여럿 들어 있는 ETF, 분석 부담 없음 + FOMO 차단.",
      },
    ],
  },
  {
    ...
    intro:
      "매수 버튼을 누르는 순간이 가장 위험하다. 사전에 만든 룰이 그 순간의 충동을 우회한다. 자동화 + 체크리스트 + 24시간 쿨다운으로 의지에 의존하지 않는 통제를 구축한다.",
    principles: [
      {
        number: "2.1",
        ...
        how: "증권사 적립식 자동 매수 설정. 매수 시점은 한 번 정한 후 변경 금지.\n거시 위기 발동 시 자동 적립 일시 중단(매수 룰 2.5 적용).",
      },
      {
        number: "2.2",
        rule: "신규 성장주 매수 전 체크리스트 3가지 필수",
        why: "감으로 매수하면 (d) 자백한 \"근거 없는 자신감\" 패턴이 그대로 작동. 사전 약속이 사후 후회를 줄인다.",
        how: "매수 전 다음 셋 모두 종이/문서에 기록.\n① DART 확정 공시 기반 회사 실적 점수\n② 진입 기준(monitor_entry/entry-configs.ts 활용)\n③ 손절 라인\n셋 중 하나라도 비면 매수 보류.",
      },
      {
        number: "2.3",
        ...
        how: "신규 매수 / 임의 매도 / 유튜버 시그널 매매 / 비중 조정 모두 결정 시점부터 24시간 지연.\n스마트폰 스톱워치로 시간 측정 동안 분석만, 매매 X.\n자동 트리거(트레일링 스탑·거시 위기 객관 트리거·분기 리밸런싱 정해진 일정)는 즉시 실행 예외.",
      },
      {
        number: "2.4",
        rule: "2단계 매매 분리 — my-stock에서 결정, 증권사 앱에서 실행",
        why: "주식창 보다 즉각 매매하는 \"무지성 매매\" 차단. 결정과 실행을 분리하면 결정 단계에서 체크리스트와 24시간 쿨다운이 강제 적용된다.",
        how: "1단계(결정): my-stock 프로젝트에서 매수 의사결정 폼 작성 + 24시간 타이머 시작.\n2단계(실행): 24시간 통과한 종목만 증권사 앱에서 매매.\n충동 매수 시 my-stock 켜고 폼 작성 자체가 5분 정도 시간을 끔.",
      },
      {
        number: "2.5",
      ...
      },
      {
        ...
        how: "시장 주간 +5% 이상 폭등 + 본인 계좌 정체 감지 시 그 주 신규 매수 금지.\n분석은 가능, 매수는 다음 주 월요일 이후.",
      },
    ],
  },
  {
    ...
    principles: [
      {
        ...
        how: "(a) 매수 ~ +10% 도달 전: 매수가 -10% 시 매도 (전통 손절).\n(b) +10% ~ +50% 도달 전: 도달 최고점 -10% 시 매도 (트레일링 스탑). 펀더멘털 무훼손 + 외인 매도 -5% 시 자동 보유 룰 적용 가능.\n(c) +50% 이후: 도달 최고점 -10% 시 매도. 펀더멘털 무관 즉시.",
      },
      {
        number: "3.2",
        rule: "매도 후 재매수 분할 + 안전장치",
        why: "손절 후 V자 반등하면 영원히 못 돌아오는 함정. 재매수 트리거를 매도 시점에 미리 정해두지 않으면 충동 결정이 됨.",
        how: "매도 시점에 다음을 캘린더·증권 앱 알림에 등록.\n매도가 -10%(1/3) / -20%(1/3) / -30%(1/3) 분할 매수.\n마지막 1/3 안전장치: 다음 셋 중 가장 먼저 발생\n(가) 매도가 -30% 도달\n(나) 매도가 -20% 도달 후 저점 +10% 반등\n(다) 매도가 -20% 도달 후 3개월 경과",
      },
      {
        number: "3.3",
        rule: "코어 코스피200 ETF 매도 금지",
        why: "코어는 FOMO 안전판이다. 코어를 흔들면 안전판이 무너진다. 단순 사이클 다운으로 -25% 빠져도 V자 반등 가능성이 있어 보유가 답.",
        how: "분기 리밸런싱(50% 비중 유지)을 위한 매도 외에는 매도 금지. 핫 섹터 진입은 성장주 영역 25% 안에서만.\n다음 두 예외만 매도 가능:\n(a) 슈퍼사이클(코스피·반도체 12개월 +50% 또는 반도체 ETF 코스피 대비 +30%p 아웃퍼폼(특정 주식의 상승률이 시장 평균보다 살짝 더 클 것이라고 예측하는 것)) + 사이클 다운 신호(코스피 -10% + 반도체 60일 이평 하향 + 외인 5거래일 순매도) 동시 충족.\n(b) 거시 위기 트리거 발동(룰 3.6).",
      },
      {
        number: "3.4",
        rule: "배당주 매도 금지",
        why: "배당주는 앵커 역할(매도 충동 억제). \"팔지 않고 오래 보게 되는 명분\"이 본인 평온의 진짜 트리거 중 하나. 자주 매매하면 그 효과 사라짐.",
        how: "분기 리밸런싱 외 매도 금지.\n예외: 펀더멘털 훼손(분기 영업이익 전년 동기 대비 -40% 이상 또는 배당 삭감 발표).\n거시 위기 트리거 발동 시 함께 매도.",
      },
      {
        number: "3.5",
        rule: "외인 매도 자동 보유 룰 (펀더멘털 무훼손 시)",
        why: "이상적 자기(시나리오 답변에서 침착)와 실시간 행동(외인 매도 보면 흔들림)이 어긋남을 본인 자백. 의지가 아니라 룰로 통제해야 함.",
        how: "매수 시점에 \"이런 일이 일어나면 내 가설이 깨진 것\"이라는 펀더멘털 훼손 기준 미리 정의(예: 분기 영업이익 전년 동기 대비 -20% 이하 + 컨센 미달).\n그 기준이 발동 안 한 상태에서 외인 순매도 + 가격 -5% 빠지면 다시 분석하지 않고 그냥 보유.",
      },
      {
        number: "3.6",
        rule: "거시 위기 트리거 발동 시 보유 자산 100% 매도",
        why: "사용자 거시 공포 본능을 룰로 흡수하되, 단순 \"감\"이 아니라 객관적 트리거로 통제. 이란-미국 전쟁 사례 데이터 검증 — \"5거래일 연속 보도\" 트리거는 3월 9일 발동 시점에 이미 -18% 손실 후. 빠른 트리거 추가 필요.",
        how: "다음 둘 중 어느 하나 충족 시 발동.\n\n**(빠른 트리거)** 셋 모두 충족:\n① 단일거래일 코스피 -5% 이상 폭락(종가 또는 시초가 갭)\n② 매일경제/한국경제 1면 위기 사유 보도\n③ Claude에게 거시 상황 보고 후 동의\n\n**(객관 트리거)** 둘 다 충족:\n① 코스피 전고점 -10% 도달\n② 다음 셋 중 하나: 전쟁 발발 / 한국 분기 GDP -2% 또는 미국 두 분기 연속 마이너스 / 환율 1,500원 돌파\n\n발동 후: 매도 자금 50% CMA/MMF, 50% 단기채 ETF 분산. 재매수 트리거(저점 +10% / 외인 5거래일 연속 순매수 / 거시 사유 해소) 충족 시 1/3씩 분할 재매수.",
      },
    ],
  },
  {
    ...
    intro:
      "FOMO 회피와 잃지 않는 투자는 양립 불가. FOMO 트리거 시점이 가장 위험한 순간이며, 그 순간을 통제하는 룰들이 헌장의 핵심.",
    principles: [
      {
        number: "4.1",
        ...
        how:
          "헌장의 모든 룰을 \"FOMO 트리거 시점에서도 작동하는가\"로 점검.\n잃지 않는 투자와 충돌하면 FOMO 회피가 우선.\n단, 영구 손실 회피(아래 정의 명확화 섹션)는 잃지 않는 투자의 본질이라 절대 양보 X.",
      },
      {
        number: "4.2",
        ...
        how:
          "친구·커뮤니티·유튜버 수익 자랑을 보면 매주 일요일 자동 리포트(연초 대비 누적 수익 + 누적 배당 + 코스피 대비 상대 성과)를 다시 본다.\n전략 변경은 분기 단위로만.",
      },
      {
        number: "4.3",
        ...
        how:
          "유튜버가 \"지금 다 파셔야 한다\" 같은 강한 시그널 발신 시 24시간 후 매매 결정.\n24시간 후에도 그 시그널이 합리적이면 매매. 보통 흥분이 가라앉고 다른 정보가 들어와 검증되어 매매 보류로 결정되는 경우가 많음.",
      },
      {
        number: "4.4",
      ...
      },
      {
        ...
        how:
          "표준 답변: \"코스피 ETF 절반 들고 있어서 시장은 따라가. 더 사면 천장이라 위험해. 우리 룰대로 가는 게 맞아.\"\n와이프께 헌장 룰북·주간 리포트는 같이 보여주되, 의사결정 참여는 안 부담.",
      },
      {
        number: "4.6",
        rule: "충동이 강할 때 거래 금지",
    ...
    principles: [
      {
        ...
        why:
          "FOMO의 다른 형태(\"내 종목이 일하는 느낌이 없다\")를 분기배당이 아니라 주간 리포트로 채운다.\n누적·상대 성과가 단기 변동을 노이즈로 처리하는 효과.",
        how:
          "5섹션:\n① 주간 변동\n② 연초 대비 누적 수익\n③ 연초 대비 누적 배당\n④ 펀더멘털 진척(주요 보유 종목 매출·영업이익 전년 동기 대비)\n⑤ 코스피 대비 상대 성과\nmy-stock에서 자동 생성.",
      },
      {
        number: "5.2",
        rule: "분기 자기검증 — 성장주 vs 코스피 ETF 비교",
        why:
          "본인 종목 선택 능력이 진짜 시장 평균보다 나은지 객관적 데이터로 확인.\n(d) 자백 객관 검증. 계속 못 이긴다는 게 데이터로 드러나면 비중을 강제로 줄여 손해 키우는 걸 막기 위함.",
        how:
          "매 분기말 성장주 영역 누적 수익률 vs 같은 기간 코스피200 ETF 비교 기록.\n4분기(1년) 연속으로 코스피 ETF보다 -5%p 이상 지면 성장주 비중을 자동 -5%p 축소.\n의지가 아니라 룰로.",
      },
      {
        number: "5.3",
        ...
        how:
          "분기말 1회 리밸런싱. 6% 초과 종목은 6%로 절삭(차익실현), 1% 미만 자투리 종목은 정리.\n코어 ETF는 50% 비중 유지를 위한 리밸런싱만.",
      },
    ],
  },
];
...
const heldItems = [
  {
    ...
    why: "밸류에이션 트리거는 슈퍼사이클 진입 종목에 못 올라타는 FOMO가 더 큰 고통이라는 본인 판단으로 채택 안 함. FOMO 회피와 일관된 선택.",
    triggers: [
      "반도체 ETF가 60일 이동평균선 하향 돌파 + 외인 5거래일 연속 순매도",
      "글로벌 메모리 반도체 현물가(DRAM/NAND) 분기 단위 하락 시작",
      "코스피 지수 전고점 대비 -15% 도달",
    ],
    next: "위 셋 중 하나 발동 시 밸류에이션 트리거 또는 시간 연장 방식으로 전환 검토.",
  },
  {
    title: "한국 시장 100% 노출",
    decision: "한국 100% 유지 (코리아 디스카운트 개선 모멘텀에 베팅)",
    why: "정부의 코리아 디스카운트 개선 정책 + 한국 시장 슈퍼사이클 모멘텀에 베팅한 결정.\n정부 정책에 의존한다는 약점은 객관적 진입 기준(환율·박스권·정책 변화)을 미리 정해두어 보완.",
    triggers: [
      "한국 시장 지지부진 — 코스피 6개월간 ±5% 박스권 횡보",
      "달러 환율 저렴 — 원달러 환율 1,200원 이하",
      "정치 변수 변화 — 코리아 디스카운트 개선 정책 후퇴 또는 정권 교체",
    ],
    next: "위 셋 중 둘 충족 시 미국 시장 진입 검토 시작.\n진입 규모:\n- 코어 50% 안에서 미국 S&P500 ETF 10~20%p\n- 배당 25% 안에서 미국 배당 ETF 5~10%p\n- 성장주 25% 안에서 미국 저평가 성장주 5~10%p\n6~12개월 분할 진입.",
  },
];
...
const rejectedItems = [
  {
    ...
    alternative: "정보 공유는 OK. 주간 리포트와 헌장 룰북은 같이 보기 — 와이프가 \"우리 자산이 잘 운용되고 있다\"는 안심을 얻을 수 있는 수준.\n의사결정 참여는 부담시키지 않음.\n와이프 통제 자리는 자동화 도구(자동 매수·캘린더 알림·24시간 쿨다운·매수 전 체크리스트 강제)가 대체.",
  },
];
  ...
  return (
    <div className="space-y-14">
      ...
      <section className="bg-surface-container-low rounded-xl p-6 sm:p-8 ghost-border">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-tertiary/5 border border-tertiary/20 rounded-lg p-4">
            ...
            <p className="text-xs text-on-surface-variant/85 leading-relaxed">
              평가손 자체보다 거시 흐름으로 원금이 깨질 것 같은 느낌이 더 큰 트리거. 시나리오 -18% 평가손에서는 침착하게 펀더멘털 점검 후 대응. → 거시 위기 트리거 + 비상금 분리 + 코어/배당 매도 금지로 통제.
            </p>
          </div>
        </div>
        <p className="text-xs text-on-surface-variant/70 mt-5 leading-relaxed">
        ...
        <div className="space-y-4">
          {heldItems.map((item) => (
            <article key={item.title} className="bg-surface-container-low rounded-xl p-6 ghost-border">
              ...
              <p className="text-xs text-on-surface-variant mb-3 whitespace-pre-line">
                <span className="text-primary/80 font-medium">현재 결정 · </span>
                {item.decision}
              </p>
              <p className="text-xs text-on-surface-variant/85 mb-3 leading-relaxed whitespace-pre-line">
                <span className="text-primary-dim/80 font-medium">왜 · </span>
                {item.why}
              </p>
              ...
              <p className="text-xs text-on-surface-variant/85 leading-relaxed whitespace-pre-line">
      ...
      <section>
        ...
        <p className="text-sm text-on-surface-variant mb-6 leading-relaxed">
          제안되었으나 본인 환경·합리적 사유로 채택하지 않은 항목. 거부 사유와 대체 방안을 명시.
        </p>
        <div className="space-y-4">
          {rejectedItems.map((item) => (
            <article key={item.title} className="bg-surface-container-low rounded-xl p-6 ghost-border">
              ...
              <p className="text-xs text-on-surface-variant/85 leading-relaxed whitespace-pre-line">
                <span className="text-tertiary/90 font-medium">대체 방안 · </span>
                {item.alternative}
              </p>
            </article>
          ))}
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="space-y-2 text-sm text-on-surface-variant leading-relaxed">
            ...
            <p className="text-xs text-on-surface-variant/60">
              ...
              의지가 약한 순간을 위해 자동화·체크리스트·24시간 쿨다운으로 통제한다.
            </p>
          </div>
        </div>
      </section>
