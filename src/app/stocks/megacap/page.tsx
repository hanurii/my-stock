...
export default async function MegacapPage() {
  ...
  const buffettCandidatesFiltered = allStocks.filter((s) => s.is_buffett_candidate).length;
  const signalFiltered = allStocks.filter(
    (s) => s.signal.label === "강한 매수" || s.signal.label === "매수 검토",
  ).length;

  const buyTriggered = allStocks
    ...
    <div className="space-y-12">
      ...
      <section>
        ...
        <p className="text-base text-on-surface-variant mt-2">
          전체 {allStocks.length}종목 · 버핏 후보 {buffettCandidatesFiltered}개 · 매수 시그널 {signalFiltered}개
        </p>
      ...
      <section>
        ...
        <ReturnMachineCards stocks={allStocks} />
      </section>

      {/* Buffett Benchmark Card */}
      <BuffettBenchmarkCard stocks={allStocks} />
      ...
      <section>
        ...
        <MegacapTable stocks={allStocks} fxData={fxData} />
      </section>
          ...
          <div>
            ...
            <div className="space-y-2">
              ...
              <PillarCard
                ...
                subtitle="Capital"
                points={20}
                question="번 돈을 잘 굴려서 주주에게 돌려주고 있는가?"
                color="#34d399"
                metrics={[
                  { label: "잉여현금수익률 ≥8% (7점)", plain: "시가총액 대비 매년 통장에 진짜 들어오는 현금이 충분 — 환원 여력의 원천" },
                  { label: "총 주주환원율 ≥10% (7점)", plain: "배당 + 자사주매입을 시총으로 나눈 값. 6% 이상 5점, 3% 이상 3점, 1% 이상 1점 — 실제로 주주에게 돌려주는 비율" },
                  { label: "주당순이익 성장률 ≥15% (3점)", plain: "1주당 이익이 매년 15% 이상 성장 (작년 1,000원 → 올해 1,150원)" },
                  { label: "매출 성장률 ≥10% (3점)", plain: "회사 자체가 매년 10% 이상 커지고 있는가" },
                ]}
                analogy="식당이 매년 진짜 현금을 만들어내고(잉여현금수익률), 그 중 상당 부분을 사장님 가족에게 배당으로 주거나 다른 동업자 지분을 사들여(자사주매입) 내 지분율을 높여주는 가게. 버핏의 애플 매입 시점 총 주주환원율은 약 8~10%였습니다."
              />
              <PillarCard
                title="가격 매력"
                subtitle="Valuation"
