...
BT = ROOT / ".cache" / "bt5y"
OUT = Path(r"C:/Users/hanul/AppData/Local/Temp/claude/C--Users-hanul-playground-my-stock/6169d0c4-0281-4b63-96b9-ea3ec7e90cf4/scratchpad/repro")
...
def main():
    ...
    n_dup = 0
    for y in YEARS:
        d = json.loads((BT / f"bt_{y}.json").read_text(encoding="utf-8"))
    ...
    gate = {g: {k: [0, 0] for k in FIELDS} for g in ("확정", "ambiguous", "unresolved")}
    mismatches = []
    missing_series = []
    path_lens = []
    n_truncated = 0
    ...
    (OUT / "01-path-build.json").write_text(
