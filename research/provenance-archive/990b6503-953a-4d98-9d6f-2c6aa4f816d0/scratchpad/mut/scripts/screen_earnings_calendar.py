...
판정 로직 (estimate_next_earnings — 순수 함수, 단위테스트 대상):
  1. confirmed: asof 21일 이내 결산실적공시예고(이후 잠정/정기 없음) → 창 [예고+1, 예고+14],
     ...
     예고 없고 10일 이내 IR개최 안내 → 창 [IR일, IR+7] (약한 신호) — 단 IR 창이
     아직 실적이 안 나온 실적시즌 창(SEASON_SPANS_MD)과 겹칠 때만. 시즌 마감 직후
     NDR·발표된 실적의 사후 설명회 IR은 예고가 아님 (26-08-20 오탐 6건 교훈).
...
def estimate_next_earnings(filings: list[dict], asof: date | str) -> dict | None:
    ...
    # 단, IR 창이 "아직 실적이 안 나온 실적시즌 창"과 겹칠 때만 발표 예고로 인정 —
    # 시즌 마감 직후 NDR·이미 발표된 실적의 사후 설명회 IR은 실적 이벤트가 아님.
    ...
    if irs:
        ...
        heralds = any(
            ...
            for s, e in _season_spans(win[0].year)
        )
