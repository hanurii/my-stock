...
**수집** 조사 세션 2026-08-29 (2026-08-30 갱신) · **수집만 했습니다. 검정·판정 없습니다.**
**용도** 100번 「미너비니 원칙 기반 수익률 검정」의 규칙 근거

## ★ 이 프로젝트의 «목표» — 사용자 확답 (2026-08-30)
> ### **「우리는 백테스팅을 통해 이러한 모델을 하나 만드는 걸 목표로 하고 있다」**

20번에서 확인되듯 미너비니의 **「$SPY 전략 모델」·「장기 기계적 모델」은 회사 기밀**이라 재현할 수 없습니다.
즉 **재현 불가능한 부분이 그의 방법 «안»에 있습니다.** 우리가 하는 일은 그것을 «복원»하는 것이 아니라
**같은 자리에 놓을 모델을 «우리 자료로» 하나 만드는 것**입니다. 이 문서는 그 재료입니다.
...
5. 현재가가 50일선 «위»
6. 현재가가 52주 «저가»보다 최소 **30% 위**  ← ✅ **확정 (사용자 확답 2026-08-30)** [1차-전사]
   (웹에는 25%로 적은 곳이 많으나 **원전은 30%** 이고, **우리 코드도 30% 입니다** — 이 항목 닫힘)
...
④ 이미 쟀나? — **쟀습니다.** 저장소의 `find-trend-template` 이 이 관문입니다. 미국 판 경로가 전부 이 관문 통과분입니다(91·93). ✅ **6번(30%)은 우리 코드와 일치 — 갈라 볼 필요가 없어졌습니다.**
...
#### ★ **"dearth of fundamentals" 의 원문 문맥 — 2026-08-30에 «앞뒤 문단»을 찾았습니다**

파워플레이 절 전체가 이렇게 이어집니다 (whatheheckaboom, 2014 · 책에서 옮긴 것):
...
>
> "This is the only situation I will enter with a dearth of fundamentals. …"

##### 🚨 물음의 답 — **둘 «중» 하나가 아닙니다**
```
② 「빈약(수준)」인가 「꺾임(기울기)」인가를 가릴 «형용사»를 찾았습니다:
   deceleration / slowing / decelerating / deteriorating  →  **하나도 없습니다**
   weak / poor / no earnings / not yet profitable         →  **하나도 없습니다**
   실적에 붙은 수식어는 **«current» 하나뿐**입니다 — "regardless of what the
   **current** earnings and sales are showing you"
```
> ### 즉 원문이 말하는 것은 **「지금 «보고된» 실적이 «무엇을 보여 주든» 상관없다」**입니다.
> ### **문턱을 «낮추는» 말이 아니라 그 축을 «안 쓰는» 말입니다.**

근거 둘:
```
· "dearth" 는 «부족·결핍»이라 «수준/유무» 쪽 낱말이고 «기울기» 낱말이 아닙니다
· 바로 앞 문장이 "**a dramatic shift in the prospects of a company**" 입니다
  → 「아직 숫자에 «안 나타난» 변화를 주가가 먼저 말한다」는 취지이고,
    그래서 «현재 숫자»를 통째로 제쳐 둡니다
```

##### 105 에 대해 — **제가 판정하지 않고 사실만 적습니다**
```
105 구현   「직전 분기에 성장률이 «꺾인» 회사」를 탈락 (성장률 50%→40% 도 탈락)
원문       파워플레이에서는 **어떤 실적 조건도 «안» 건다**
→ 원문은 「빈약」도 「꺾임」도 아니라 **«면제»**입니다.
   두 선택지 중 하나를 고르는 물음이었다면 **답은 「둘 다 아니다」**입니다.
```

##### ③ 책 «밖»에서 같은 취지 — ⚠️ **못 찾았습니다**
「파워플레이는 펀더멘털 없이 기술적으로만 접근할 수 있는 유일한 패턴」이라는 문장이 여러 곳에 돌지만 **전부 이 책 대목을 다시 쓴 [3차-해석]**이고, **트윗·인터뷰의 본인 말은 없습니다.**

##### ④ 한국판 p.320~322 대응 — ⚠️ **확인 불가**
위 대목이 원서 파워플레이 절이라는 것까지는 확실하나, **원서 쪽수를 확인할 방법이 없습니다.** (책 PDF 링크는 열지 않았습니다.) 사용자가 한국판 p.320~322 에 위 세 조건(100%/8주 · 20~25%/3~6주 · 10% 또는 VCP)과 "dramatic shift in the prospects" 문장이 있는지 보시면 대조가 됩니다.
...
#### ✅ 손절을 «어떻게 집행하는가» — 확정 (사용자 전사, 2026-08-30) [1차-전사]
『Momentum Masters』:
> 「주가가 스톱 가격을 만나면 **전체 포지션을 즉시 매도**한다. 때때로 **평균 손실률이 스톱 한 개만
>  설정할 때와 «일치»하도록** 스톱을 여러 단계로 쌓아 놓기도 한다. … 한 번에 조금씩, 보통
>  **3분의 1 또는 반씩** 손실이 낮아진다」

```
기본        **전량 손절** (분할 손절이 아닙니다)
변형        스톱을 여러 단계로 쌓기 — 한 번에 **1/3 또는 1/2** 씩
```
> ### 🚨 **스톱 쌓기는 「손실을 «줄이는»」 장치가 «아닙니다».**
> ### 원문이 **「평균 손실률이 스톱 한 개일 때와 «일치»하도록」** 이라고 못 박습니다 →
> ### **기대값은 «불변»이고, 줄어드는 것은 «분산»입니다.**

**→ 백테스트에서 「분할 손절이 전량 손절보다 낫다」가 나오면 그건 원전 주장이 아닙니다.**
④ 이 형태는 **미측정.** (24번 청산 격자는 전량 손절 하나만 걸었습니다.)
...
#### ✅ **«내림»은 확정됐습니다 — 「한 번만 져도 한 칸」** (사용자 예시 재전사, 2026-08-30) [1차-전사]
```
1거래 ¼   성공  →  2거래 ½   성공  →  3거래 «전체» **실패**  →  4거래 ½ **실패**  →  …
```
> ### **올림 = 한 번 이기면 +1칸 · 내림 = «한 번만 져도» −1칸.** 비대칭입니다.

🚨 **99번이 「우리 구성」이라 라벨을 달았던 «한 칸 내림»이 «원전 것»으로 확정됐습니다.**
그 라벨을 고쳐야 합니다. (올림 쪽 «단 수»가 4단인지는 여전히 미확정 — 위 경고 유지)
...
### 20. 「기계적 모델」이 따로 있다 — 🔒 **영구 미확인으로 «확정»** (사용자 확답 2026-08-30)
① 인용 — [1차-본인]
> "Our **long term mechanical model** has not yet triggered a sell signal…" (X id 1874139838313574883)
> "Our **$SPY strategy model** … to **match the S&P 500 on the upside, while avoiding the big downside of bear markets and large corrections**…" (X id 1846266751341985986)

② 🔒 **회사 기밀입니다. 「아직 못 찾았다」가 아니라 «찾을 수 없다»입니다.**
```
→ **재현 불가능한 부분이 그의 방법 «안»에 있습니다.**
→ 그러므로 「원전대로 했는데 성적이 다르다」를 «이 모델 탓»으로 돌릴 수도, 배제할 수도 없습니다.
   결과 문서에 **「그의 실적에는 우리가 못 보는 모델이 하나 들어 있다」**를 한계로 적으십시오.
```
★ 그리고 이것이 이 프로젝트의 목표를 정합니다 — **문서 머리말** 참조.
