...
FLOOR_BE = True
# ⛔ 252 «전용» 복사본이다 — 원본은 `pyr_trigger.py`.
#    MA_N = 0 ⇒ «원본» 그대로(25일 저가 채널) · MA_N = 50 ⇒ «50일선» 트레일.
MA_N = 0
...
def _phase2(p, i, a, half, trail, ft, fs, epx, lots, sched, stop, tpx):
    ...
    for j in range(i + 1, n):
        if MA_N > 0:
            # 252 — «이평선» 트레일: 「어제까지」의 종가 MA_N 일 평균.
            seg = [x for x in c[max(0, j - MA_N):j] if x is not None]
            lvl = (sum(seg) / len(seg)) if seg else None
            s2 = (a if FLOOR_BE else 0.0) if lvl is None else (
                max(a, lvl) if FLOOR_BE else lvl)
        else:
