...
def main():
    ...
    print("   차입료 = 차입 %.0f%% × 크기 %.0f%% × 숏 든 날 %.1f%%  =  **%.4f%%p/년**"
          % (BORROW, SHORT_ON * 100, frac0 * 100, bcost0))
    print("   이 값은 **«씨앗과 무관하게» 반드시 빠진다** — 숏은 «같은 거래» 위의 «덧씌우기»다")
    print("   ⇒ **60/60 은 「자료가 말한 것」이 아니라 「구조가 «정한» 것」**이다")
    print("")
    print("★ 검사 — **「효과 ÷ 씨앗 SD」가 클수록 그 p 는 «정보가 없다»**")
    print("   («결정적 기전»을 통계로 «다시» 잰 것이기 때문)")
    print("```")
    print("")
    ...
    if -DELTA < lo and hi2 < DELTA:
        print("⇒ ⛔ **「동등성 검정을 «통과»했다」로 적지 «않는다»** — «성취»처럼 읽힌다")
        print("   CI 폭 **%.4f%%p** · Δ 가 그 폭의 **%.0f배** · 효과는 Δ 의 **1/%.0f**"
              % (hi2 - lo, DELTA / (hi2 - lo), DELTA / md))
        print("   ⇒ **«통과했다»가 아니라 «질 수가 없었다»**")
        print("✅ **적을 말: 「이 크기(%.3f%%p)는 «결론을 바꾸는 크기»(%.2f%%p)의 «1/%.0f» 라"
              % (md, DELTA, DELTA / md))
        print("   «애초에 위협이 아니었다»」  ⇒ **150·152·154 는 «안 뒤집힌다»**")
    elif lo <= -DELTA and hi2 >= DELTA:
        print("⇒ 🚨 **구간이 ±%.2f 를 «양쪽으로» 넘는다  →  「거의 같다」가 «아니라» 「못 가린다」**" % DELTA)
    else:
        print("⇒ 🚨 **구간이 ±%.2f 를 «벗어난다»  →  「같다」로 못 적는다. «왜»부터 본다**" % DELTA)
    dm = [B["mdd"][i] - A["mdd"][i] for i in range(n_seed)]
    mm, sm = st.mean(dm), st.stdev(dm)
    ...
    print("   관측 **%.3f%%p/년**  →  **차입료 %.0f%%** (회계 · 검정 «불필요»)"
          % (md, 100 * bcost / md))
    print("   나머지 **%.3f%%p (%.0f%%)** — 🚨 **«안 갈랐다».** 복리·세금 상호작용·숏 손익이"
          % (md - bcost, 100 * (md - bcost) / md))
    print("   **«다 섞인» 잔차다. ⛔ 「숏 자체 때문」처럼 «이름»을 붙이지 «않는다»**")
    print("```")
    print("")
