...
def main():
    ...
    if capped:
        ...
        pos = [x for x in capped if x[5] >= 0]
        if pos:
            P("")
            P("     🚨 **양수 %d 개 — «이름»과 «크기»:**" % len(pos))
            for x in sorted(pos, key=lambda z: -z[3])[:12]:
                P("        %-7s %s  배당 몫 **%+.1f%%** · 가격 수익 **%+.1f%%**"
                  % (x[0], x[1], 100 * x[3], 100 * x[5]))
            P("")
            P("     ★★ **둘로 갈린다 — 그리고 뒤쪽은 «내 관문의 결함»이다:**")
            big = [x for x in pos if x[3] >= 0.20]
            sml = [x for x in pos if x[3] < 0.20]
            P("     ⓐ **큰 것 %d 개**(몫 ≥20%%) — **분할상장·청산분배**로 보인다:" % len(big))
            P("        **GE 2024-04**(GE Vernova 분할) · **BBWI 2021-08**(L Brands 분할) ·")
            P("        **GGP 2011-07**(Rouse 분할·파산 후 재편) · **FTAR 2007-04**(청산분배)")
            P("        ⇒ 분할상장은 «가격»이 «안» 떨어진다 — 가치가 «새 회사»로 갔고 `close` 는 그걸 «안» 담는다")
            P("        🚨 **이것도 «내가 알아본» 것이다. «잰» 것이 아니다**")
            P("     ⓑ **작은 것 %d 개**(몫 %.1f~%.1f%%) — 🚨 **«정상 배당»이 «내 상한»에 걸린 것**:"
              % (len(sml), 100 * min(x[3] for x in sml), 100 * max(x[3] for x in sml)))
            P("        상한 = 연 25% × (보유일/365) 이라 **짧게 들면 상한이 «아주 작아진다»**")
            P("        (30일 보유 → 상한 **2.05%**. 분기 배당 한 번이면 «넘는다»)")
            P("        ⇒ ⛔ **DB★ 는 «연 환산» 상한이라 «짧은 보유»에 «잘못» 문다. 내 설계 결함이다**")
            P("        ✅ 고칠 방향 — **«한 사건»의 절대 상한**(예: 30%)으로 바꾸거나 «보유일 하한»을 둔다")
