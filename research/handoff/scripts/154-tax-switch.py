...
def main():
    ...
    print("   ★ 조합별 상한 — (3,0) **0.250** · (4,1) 0.375 · (5,2) 0.453 · (6,3) 0.508 …")
    print("     **0.05 «근처»에도 못 간다.** 자료를 «한 줄도 더 안 보고» 알 수 있었다(149 와 같은 수법)")
    print("```")
    print("")
    if (b, cN) in ((0, 3), (3, 0)):
        print("> ### 🚨🚨 **그리고 실측이 «가장 유리한» 조합 (3,0) 에 «정확히» 앉았다.**")
        print("> ### **⇒ 「«가장 유리한 배치»가 «실제로» 일어났는데도 못 가린다」 —**")
        print("> ### **   이건 「이 자료로는 못 가린다」의 «가장 센» 형태다. «운이 나빠서»가 «아니다».**")
        print("> ### ⛔ **그러니 「판을 늘리면 되지 않나」도 «여기서» 막힌다** — 배치가 이미 최선이었다")
        print("")
    print("")
    ...
    print("### 🚨 관문 V★ — **«항등식»을 코드에 박았다** (검증 세션 정정: 범위가 아니라 «항등식»이다)")
    print("")
    print("```")
    print("제약의 «참이름» —  **연 실현 합  +  창 끝 미실현  =  총 이익**   (`124:99` 가 강제한다)")
    print("   ⇒ 따라서 「창 끝 미실현 비중」은 **[0, 100]%**. 이건 «범위 말»로 적은 «항등식»이다")
    for tg in TARGETS:
    ...
    print("```")
    print("")
    print("### 🚨 관문 W★ — **«자릿수»를 박는다** (오늘 «두 번» 틀린 바로 그 종류)")
    print("")
    print("```")
    print("「연 실현이익 중앙」은 **1,000만원 계좌**의 한 해 실현이라 **10⁰ ~ 10⁴ 만원** 자리여야 한다")
    for tg in TARGETS:
        v = st.median(yreal[tg])
        ok = 1.0 <= abs(v) <= 1e4 if v else True
        print("   +%-3.0f  **%.1f만**   %s" % (tg, v, "자릿수는 그럴듯" if ok else "🚨 **자릿수 밖**"))
    print("🚨 **그런데 「자릿수가 그럴듯」이 «맞다»는 뜻은 «아니다»** —")
    print("   오늘 틀린 둘(**1000배** · **lsc 배율**) 중 **1000배는 이 관문이 잡았을 것**이고,")
    print("   **lsc 배율(1.36배)은 «자릿수 안»이라 «못» 잡는다.** 그건 **V★(항등식)**이 잡았다")
    print("★ 그래서 관문은 «하나»로 안 되고 넷을 «차례로» 묻는다:")
    print("   ① 범위  ② **항등식**  ③ **자릿수**  ④ **두 경로**(= R★)")
    print("   ★ **「운이다」라고 부른 자리마다 대개 «안 건 관문»이 있다**")
    print("```")
    print("")
