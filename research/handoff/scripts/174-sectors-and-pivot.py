...
def main():
    ...
    P("   중앙 **%.4f** ⇒ 피벗이 전일 고점의 **%.1f%%** 자리다" % (allv[len(allv) // 2],
                                                          100 * allv[len(allv) // 2]))
    P("")
    P("## ⇒ ★★ **«검출기별» 위치가 «코드 읽기»와 «맞는다» — «따로» 읽는다:**")
    for pat in sorted(by_pat):
        v = sorted(by_pat[pat])
        m_ = v[len(v) // 2]
        src_ = "**«종가»**" if pat.upper().startswith("VCP") else "**«장중 고가»**"
        side = "**«아래»**" if m_ < 1.0 else ("**«위»**" if m_ > 1.0 else "«같음»")
        P("   %-4s 중앙 **%.4f** ⇒ 전일 고점 %s  ·  피벗의 «자» = %s"
          % (pat, m_, side, src_))
    P("")
    P("   ## ⇒ ✅ **VCP 가 «아래», 3C·PP 가 «위» — «코드»가 말한 그대로다**")
    P("     **«종가»**는 «고가»보다 «작거나 같다** ⇒ VCP 피벗이 전일 고점 «아래»에 앉는다")
    P("     **«구간» 고가**는 «전일» 고가보다 «크거나 같다** ⇒ 3C·PP 가 «위»에 앉는다")
    P("   ⇒ ★ **«코드 읽기»와 «수»가 «따로» 같은 말을 한다** — 둘이 «맞물리니» 라벨이 «선다**")
    P("")
    P("🚨🚨 **「우리 «판»들이 «세 검출기»를 «섞어» 썼는가」 — «셌다»:**")
    P("")
    P("| 검출기 | n | 후보 중 |")
    P("|---|---:|---:|")
    for pat in sorted(by_pat, key=lambda k: -len(by_pat[k])):
        P("| **%s** | %s | **%.1f%%** |"
          % (pat, format(len(by_pat[pat]), ","), 100.0 * len(by_pat[pat]) / len(allv)))
    P("")
    P("## ⇒ 🔴 **«섞어» 썼다** — `165`·`166`·`167` 은 **후보 «전부»**를 쓴다")
    P("   ⇒ 그 판들의 **「피벗」은 «한 낱말»이 «아니었다»** —")
    P("     **VCP %.1f%%(«종가») + 3C %.1f%%(«고가») + PP %.1f%%(«고가»)**의 «섞임»이다"
      % (100.0 * len(by_pat.get("VCP", [])) / len(allv),
         100.0 * len(by_pat.get("3C", [])) / len(allv),
         100.0 * len(by_pat.get("PP", [])) / len(allv)))
    P("   🚨 **판정이 «바뀌는지»는 «안» 쟀다** — 「검출기별로 갈라 다시 돌리기」는 **«다른 판»**이다")
    P("   ✅ 그래도 **「한 낱말이 «아니었다»」는 «사실»**이니 그 판들에 **«라벨»**을 달아야 한다")
    P("")
