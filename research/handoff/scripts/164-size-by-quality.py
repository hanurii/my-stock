...
def main():
    ...
    P("⇒ **%.1f 배** — 자산에 «비례»하므로 «커진다»(상한 20%%가 «비율»이라 «당연»하다)" %
      (st.median(l_) / max(st.median(e_), 1e-9)))
    P("🚨 **씨앗 «하나»(0번)의 수다** — 「몇 배」를 «크기»로 인용하지 않는다. «방향»만이다")
    P("★ 그리고 이건 **「유동성 부담」이 «말년»에 «훨씬» 크다**는 뜻이다 — 이 판은 그걸 «안 쟀다»")
    P("```")
    P("")
