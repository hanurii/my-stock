...
def _phase2(p, i, a, half, trail, ft, fs, epx, lots, sched, stop, tpx, be=None):
    """절반 판 뒤 — 본전(평균단가) + `trail`일 저가 추격. (47번 `_phase2` 와 같은 규약)"""
    l, c, d = p["l"], p["c"], p["d"]
    n = len(c)
    be = a if be is None else be        # 🆕 181 — «본전»선을 «따로» 준다(기본 = 평균단가)
    ex = [(d[i], half, tpx)]
    ...
    for j in range(i + 1, n):
        ...
        if not seg:
            s2 = be if FLOOR_BE else 0.0
        else:
            s2 = max(be, min(seg)) if FLOOR_BE else min(seg)
...
def resolve_one(path, mask, *, ft="limit", fs="market", stop=8.0, target=20.0,
                ...
                min_days=2, add_stop="floor_entry", h_lag=False, stay_on="low", be_px=None,
...
def resolve_all_masks(path, *, ft="limit", fs="market",
                      ...
                      add_stop="floor_entry", px_round=None, h_lag=False, be_px=None,
