...
def main():
    ...
    P("")
    P("★ **「메우는 비율」 — 🚨 «부호가 다르다»를 «같은 줄»에:**")
    P("   `166`  |사는 값| ÷ |거른 값| = 7.419/5.937 = **125%** ← 「거른 **«이득»**」을 «넘게» 먹는다")
    P("   `167a` %.3f/%.3f = **%.0f%%**  ·  `167b` %.3f/%.3f = **%.0f%%**  ← 「고른 **«손해»**」를 «못» 메운다"
      % (dif("Ⓗa", "Ⓗ′")[0], abs(dif("Ⓗ′", "Ⓓ′")[0]),
         100.0 * dif("Ⓗa", "Ⓗ′")[0] / abs(dif("Ⓗ′", "Ⓓ′")[0]),
         dif("Ⓗb", "Ⓗ′")[0], abs(dif("Ⓗ′", "Ⓓ′")[0]),
         100.0 * dif("Ⓗb", "Ⓗ′")[0] / abs(dif("Ⓗ′", "Ⓓ′")[0])))
    P("   🚨 **두 비율은 «같은 뜻»이 «아니다»** — `166` 의 「사는 값」은 **«비용»**,")
    P("     `167` 의 「늦게 사는 값」은 **«이득»**이다")
    P("   ⇒ ✅ **「양쪽 다 «상쇄가 «모자란다»»」로만 쓴다.** 「같은 자」라고 쓰면 **유형 67** 이다")
    P("")
    ...
    P("   ⛔ 「원전이 맞다/틀리다」 — 특히 **Ⓗb 가 지면 「«이 자»로는 안 된다」**다(약점 ①)")
    P("   ⛔ **Ⓗa 를 「풀백」이라 부르기** — «재돌파»다(약점 ③)")
    P("   ⛔ 「원전 «풀백 매수»가 맞다/틀리다」 — **«절반»만** 잰다")
    P("   ⛔ **「`166` 보다 «낫다/못하다»」** — 고른 «집합»이 **«다르다»**")
    P("     🚨 그리고 **«상보»도 «아니다»** — NH★ 가 「==」 %s 건(%.1f%%)이 **«어느 쪽에도» 없다**를 냈다"
      % (format(n_eq, ","), 100.0 * n_eq / nall))
    P("   ⛔ 「51.0%가 원전과 «일치»」 — **자가 «셋»**이다(51.0 / 70.4 / 85.9)")
    P("   ⛔ **「종가<피벗 = 51.0%」** — **47.6%** 다. 51.0% 는 「`166` 이 «거르는» 비율」이다")
    P("   ⛔ **판정을 「Ⓗ−①」로** — Ⓗ 는 거래가 «절반»이라 «빼는 비용»이 섞인다(㉲)")
    P("   ⛔ 「다른 창(3일·10일)도 재 보자」 — **«당일»로 «고정»**했다(약점 ②)")
    P("")
