...
CACHE = Path(str(r91.OUT / ("167%s-partial.json" % ("_" + DET.replace("3C", "c").replace("PP", "p").replace("VCP", "v") if DET else ""))))
...
def main():
    ...
    for lab, x, y in rows:
        ...
        if lo > DELTA:
            ...
            # 🔴 한때 「칸3」으로 적었다 — **정본 판정표에 «없는» 칸**이다(단일은 1·2·4a·4b·5)
            vd = "**4a** ⚠️ **«확실히» «음수»인데 «크기»가 Δ 미만**"
        else:
            vd = "🚨 **못 가린다**"
        # 🚨 CI 폭이 «사실상 0» 이면 «판정칸»을 «못» 붙인다 —
        #    「60판이 «전부» 같은 값」이라 CI 가 «잴 것»을 «안» 잰 것이다(후보 < 칸 5)
        if (hi - lo) < 1e-9:
            vd = "🚨 **판정칸 «못» 붙임 — CI 폭이 «0»**(60판이 «전부» 같은 값)"
    ...
    P("   **Ⓓb 를 «따로» 맞추니 %+.3f%%p**(**4a** ⚠️「«확실히» «음수»인데 «크기»가 Δ 미만」)로"
      % dif("Ⓗb", "Ⓓb")[0])
    P("   **«판정 «칸»이 바뀐다»**")
