...
FLOOR_BE = True
# ⛔ **257 «전용» 복사본**이다 — 원본은 `pyr_trigger.py`.
#    `CB_N = 0` ⇒ **원본 «그대로»**(목표 «전» 손절선 «고정»)
#    `CB_N > 0` ⇒ **«조건부» 본전**: 「미실현 수익 ≥ 위험노출 × CB_N」이 «닿은» «다음» 봉부터
#                 목표 «전» 손절선 `S` 를 **본전 `a`** 로 «올린다».
#    🚨 위험노출 = `a × stop/100` ⇒ **문턱 = 수익률 `CB_N × stop%`**(우리 정의 · `257-PRE` §④)
#    ⛔ **「닿은 «다음» 봉부터」** — «같은» 봉에 «올리면» 장중 «앞뒤»를 «모르는» 채 쓰게 된다
CB_N = 0.0
...
def resolve_one(path, mask, *, ft="limit", fs="market", stop=8.0, target=20.0,
    ...
    # 257 — «첫» 도달 자리(고가 기준). `shares=(1.0,)` 라 트랜치가 «하나» ⇒ `a` = `epx`
    _cb_i = None
    if CB_N > 0.0:
        _thr = epx * (1.0 + CB_N * stop / 100.0)
        for _j in range(n):
            if h[_j] is not None and h[_j] >= _thr:
                _cb_i = _j
                break
    ...
    for i in range(n):
        ...
        # 257 — «조건부» 본전: «닿은» «다음» 봉부터만 «올린다**(장중 앞뒤를 «안» 본다)
        if CB_N > 0.0 and _cb_i is not None and i > _cb_i:
            S = max(S, a)
        T = a * (1 + target / 100)
