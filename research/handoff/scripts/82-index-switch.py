...
def main() -> int:
    ...
    print("관문 ②″ 진짜 깃발(구간 %d개)로 «지수배의 곱» %.6f 을 맞히는가 — "
          ...
          flush=True)
    # 🚨 위 %.1e 는 «약한 돌연변이»(재개일 «하나»만 지움) 기준이다. **결함의 실제 크기가 아니다.**
    #    진짜 곡선은 구간 안이 시작일 하나뿐이었으므로 실제 결함은 «구간 안을 전부 지운» 쪽
    #    ≈ 3.5e-01 이다(검증 세션 실측). 문턱 1e-3 은 약한 쪽 기준으로도 43배 여유라 안전하다.
    #    → 약한 돌연변이를 잡으면 강한 것도 잡히므로 관문으로는 이쪽이 «더 엄하다».
    print("        (⚠️ %.1e 는 «약한 돌연변이» 기준 — 결함의 실제 크기는 ≈3.5e-01. "
          "문턱 1e-3 대비 %.0f배 여유)" % (e_hole, e_hole / 1e-3), flush=True)
    if not ok2:
        return 3
