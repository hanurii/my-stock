...
CACHE = Path(str(r91.OUT / ("166%s-partial.json" % ("_" + DET.replace("3C", "c").replace("PP", "p").replace("VCP", "v") if DET else ""))))
MA_REF = 12377
...
def main():
    ...
    P("=" * 104)
    P("")
    P("## 🔴🔴 **머리 — 이 판은 «강건»하지 «않다**(`179` 가 «잰» 것)")
    P("")
    P("```")
    P("① ## **«현행 구성»에서만 «참»이고 «부분»으로 좁히면 «뒤집힌다» ⇒ «강건»하지 «않다**")
    P("   (검출기를 VCP «만»·3C «만»으로 좁히면 ★판정 짝의 «칸»이 바뀐다)")
    P("")
    P("② ## **«왜»: 이 판의 짝은 «수»만 맞추고 — **«집합»은 «안» 맞췄다**")
    P("   플라세보가 「같은 «수»를 «무작위»로 버림」이라 **「«누가» 버려지는지」가 «구성»에 «딸린다»**")
    P("   (`175`·`176` 은 «집합»을 맞췄고 — 그쪽은 «부호»가 «살아남았다»)")
    P("")
    P("③ ## **판정은 «안» 내린다** — 우리는 검출기를 **«전부»** 쓰고 **«원본»이 «정본»**이다")
    P("```")
    P("")
    if DET:
        P("")
    ...
    P("```")
    P("")
    P("### 🚨 **「값을 «더» 치른다」를 «두 조각»으로 — «갭과 «같은 자»»(거래당 %)로 잰다**")
    P("")
    P("```")
    P("🚨 **자를 «맞추는» 것이 «핵심»이다**(유형 67) —")
    P("   `Ⓕ−Ⓕ′ = −7.419%p` 는 **«연환산 %p»**이고 갭 중앙 `+0.041%` 는 **«거래당 %»**다")
    P("   ⇒ **«뺄셈»으로 «바로» 안 나온다.** 그래서 **«둘 다» «거래당 %»로 «다시»** 잰다")
    P("")
    ta, tb, tc = [], [], []
    for y in keep:
        for p_ in keep[y]:
            if not (has_next(p_) and confirmed(p_)):
                continue
            pv, o0 = p_.get("pivot"), (p_.get("o") or [None])[0]
            ep = p_["entry_price"]
            c0, o1 = p_["c"][0], p_["o"][1]
            if not ep or ep <= 0 or not c0 or c0 <= 0:
                continue
            ta.append(c0 / ep - 1.0)          # ㉠ 「종가 − 체결가」 = Ⓖ 가 «치르는» 값
            tb.append(o1 / c0 - 1.0)          # ㉡ 「시가 − 종가」   = Ⓖ 가 «피하는» 갭
            tc.append(o1 / ep - 1.0)          # 합  「시가 − 체결가」 = Ⓕ 가 «더» 치르는 값
    for v in (ta, tb, tc):
        v.sort()
    P("| 조각 | 중앙 | 평균 | P10 | P90 |")
    P("|---|---:|---:|---:|---:|")
    for nm, v in (("**㉠ 「종가 − 체결가」** = Ⓖ 가 «치르는» 값", ta),
                  ("㉡ 「시가 − 종가」 = Ⓖ 가 «피하는» 갭", tb),
                  ("**합 「시가 − 체결가」** = Ⓕ 가 «더» 치르는 값", tc)):
        P("| %s | **%+.3f%%** | %+.3f%% | %+.3f%% | %+.3f%% |"
          % (nm, 100 * v[len(v) // 2], 100 * st.mean(v),
             100 * v[len(v) // 10], 100 * v[9 * len(v) // 10]))
    P("")
    P("⇒ ★ **㉠ 이 «압도적»이다** — 중앙 %+.3f%% vs 갭 %+.3f%% = **약 %.0f 배**"
      % (100 * ta[len(ta) // 2], 100 * tb[len(tb) // 2],
         abs(ta[len(ta) // 2] / max(abs(tb[len(tb) // 2]), 1e-12))))
    P("⇒ ✅ **그러므로 「하루 늦게 사는 값」의 «거의 전부»는 «갭»이 아니라**")
    P("  **«돌파일 종가가 «이미» 체결가보다 «높다»»는 데서 온다**")
    P("")
    P("🚨 **그래서 Ⓖ(「그날 «종가»에 산다」)를 «돌려도» 되찾을 몫이 «작다»:**")
    P("   Ⓖ 는 ㉡(**%+.3f%%**)만 «피하고» ㉠(**%+.3f%%**)은 **«그대로» 치른다**"
      % (100 * tb[len(tb) // 2], 100 * ta[len(ta) // 2]))
    P("   ⇒ ⛔ **«돌리지 «않는다»».** 「갭이 작으니 Ⓖ 로 되찾을 게 «적다」」가 **«수»로 나왔다**")
    P("")
    P("🚨🚨 **그리고 Ⓖ 는 «집행 방식을 «바꾸는»» 제안이다 — «먼저» 적는다:**")
    P("   사용자 방식은 **«장 시작 «전» 예약»**이고(`entry-execution-method`),")
    P("   「그날 «종가»에 산다」는 **«장 마감에 사람이 붙어야»** 한다 ⇒ **«다른 일»**이다")
    P("```", flush=True)
    ...
    P("   ㉱ 「Ⓕ 는 «덜» 거를 것」  →  🔴 **실측 «%.1f%%» vs α «23.3%%» — «두 배 넘게» 거른다**"
      % (100.0 * preF / nall))
    P("     🚨 **«분모»를 밝힌다**(규약 ⑦ — 같은 것을 가리키는 수가 «둘»이면 멈춘다):")
    P("       **%.1f%%** = `judge` «통과분» %s 개 기준  ← **이 판의 정본**"
      % (100.0 * preF / nall, format(nall, ",")))
    P("       50.8% = «거르기 «전»» 후보 24,995 개 기준(선행 계산에서 쓴 수)")
    P("       ⇒ ★ **두 수는 «같은 것»을 «다른 분모»로 센 것**이다. «둘 다» 맞고 «분모»가 다르다")
    ...
    P("★★ **세 수를 «이렇게» 읽는다:**")
    P("   **Ⓕ′−Ⓓ** = 「확인으로 «거른» 값」")
    P("     🚨 **이건 «천장»이 «아니다»** — `165` Ⓔ 와 «격»이 다르다:")
    P("       `165` Ⓔ  「α 에 «도달할지»」  = **«미래»** → **영영 «못 씀»**        ⇒ **«천장»**")
    P("       `166` Ⓕ′ 「종가가 피벗 위인가」 = **«과거»** → **«그 가격»에만 못 삼** ⇒")
    P("         🆕 **「«시점»만 바꾸면 «일부» 닿는 값」**")
    P("   **Ⓕ−Ⓕ′** = 「하루 «늦게» 사는 값」")
    ...
    P("")
    P("★★★ **α(`165`)와 «나란히» — 「어느 쪽이 낫다」가 «아니라» «공통 구조»를 본다**")
    P("")
    P("| 방법 | 거른 값 | 사는 값 | 남는 것 | **사는 값 ÷ 거른 값** |")
    P("|---|---:|---:|---:|---:|")
    P("| **α 0.65%%**(`165`) | +3.738 | −2.565 | **+1.173** | **%.0f%%** |" % (100 * 2.565 / 3.738))
    P("| **확인**(`166`) | %+.3f | %+.3f | **%+.3f** | **%.0f%%** |"
      % (a2, a1, a3, 100 * abs(a1) / abs(a2)))
    P("")
    P("## ⇒ **「둘 «다» 거르는 힘이 «크고», 둘 «다» 사는 값이 «그만큼» 먹는다」**")
    P("   α 는 **%.0f%%** 를 먹고 «남기고», 확인은 **%.0f%%** 를 먹고 «넘친다»"
      % (100 * 2.565 / 3.738, 100 * abs(a1) / abs(a2)))
    P("   ⇒ ★ **「어느 쪽이 «낫다»」가 아니라 「«같은 벽»에 «두 번» 부딪혔다」**")
    P("   ✅ 이 «비율»은 **«같은 판 «안»»의 두 조각**이라 **유형 68 에 «안» 걸린다**")
    P("   ⛔ **「확인이 α 보다 «못하다»」는 «안» 쓴다** — 거른 «양»이 51.0% vs 23.3% 로 «강도»가 다르다")
    P("")
    ...
    P("   ✅ **「조합(종가 확인 «+» α)은 «안 쟀다» — 원전도 «혹은»이라 했다」**")
    P("")
    P("★ **판정표를 «누적»한다 — 「이번엔 «안» 걸렸다」 ≠ 「다음엔 «안» 걸린다」**")
    P("   이 판에서 **새 칸**(「Ⓕ−Ⓓ > 0 ∧ Ⓕ−① ≤ −Δ」)은 **«안» 걸렸지만 «둔 채»** 뒀다")
    P("   ⇒ ✅ **「판정표를 «한 곳»에 두고 «판마다» «참조»한다」** — **유형 43 의 «판정표» 판**")
    P("   ★ 판마다 «새로» 쓰면 **«앞 판에서 배운 칸»이 «사라진다»**")
    P("```")
    P("")
    P("```")
