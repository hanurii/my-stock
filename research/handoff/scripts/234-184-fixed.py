# -*- coding: utf-8 -*-
r"""184 — **`177`(갈아타기)을 «자료» 축으로** · 두뇌 의뢰 2026-09-06

  🔴 **내 어림이 «틀렸다»** — 「«경로»를 «다시» 만들어야 하니 «새» 기계 · «반나절»」이라 «적었다».
     두뇌 ㉢: 「`183`/`185` 는 «어떻게» 했나? «같은» 문제를 «이미» «푼» 것이 «아닌가»?」
     ⇒ ✅ **맞다**. `183` 은 «경로»를 «부트»하지 «않고** 거래를
        `(진입일, 보유일, 순수익)` 로 «접어» **«날짜축»만** 다시 잘랐다.
     ⇒ ★ **「경로」가 «필요»했던 게 «아니라» 「«경과일»의 «함수»」가 «필요»했다.**
     ⇒ 🚨 **「이미 «만든» 도구를 «먼저» 본다」의 «일곱 번째» — 이번엔 «내가» 만든 것을 «내가» «안» 봤다**
...
# «씨앗» 축의 값 — `177` 문서에서 «그대로»
SEEDAX = {"Ⓡ−Ⓓ": 1.023, "Ⓓ−①": -21.017, "Ⓡ−①": -19.993}
...
def boot_swap(items, n_pos, mode, rnd, slots=SLOTS):
    """🆕 **부트한 «날짜축» «위»에서 「갈아타기」를 «굴린다».**

    items[p] = [(hold, net, u, x, key)] — 그 자리에서 «사는» 거래들
      u[k] = 경과 k일의 **«전날 종가»** 기준 미실현%   ← 「누구를 내보낼지」의 «자**
      x[k] = 경과 k일의 **«시가»** 기준 실현%          ← «내보낼 때» 받는 값
    mode: "off"(=①) · "low"(=Ⓡ · 제일 낮은 것) · "rand"(=Ⓓ · 무작위)

    🚨 **`_sim_swap.py` 와 «같은» 규약**: 결정은 «전날» 값(u)으로 · 집행은 «오늘» 값(x)으로
       · «오늘» 산 것은 «오늘» 안 판다.
    """
    eq, held, n_sw = 1.0, [], 0
    for p in range(n_pos):
        ...
        if not c:
            continue
        for hold, net, u, x, _k in c:
            if len(held) >= slots:
                if mode == "off":
                    break
                cand = []
                for idx, h in enumerate(held):
                    e = p - h["p0"]
                    if e < 1 or e >= len(h["u"]) or e >= len(h["x"]):
                        continue
                    cand.append((h["u"][e], idx))
                if not cand:
                    break
                pick = cand[rnd.randrange(len(cand))] if mode == "rand" else min(cand)
                h = held[pick[1]]
                e = p - h["p0"]
                eq += h["w"] * h["x"][e] / 100          # «오늘 시가»에 판다
                held.pop(pick[1])
                n_sw += 1
            wgt = eq / slots
            held.append({"p0": p, "end": p + hold, "w": wgt, "net": net, "u": u, "x": x})
    for h in held:
        eq += h["w"] * h["net"] / 100
    return (eq - 1) * 100, n_sw
...
def main():
    ...
    P("=" * 104)
    P("184 — **`177`(갈아타기)을 «자료» 축으로** · 부트 %s판%s"
      % (format(nboot, ","), "  🚨 **--quick**" if quick else ""))
    P("=" * 104)
    P("")
    P("> 조사 세션 · 2026-09-06 · `scripts/184-swap-data-axis.py` · **문서는 이 출력 그 자체**(유형 48)")
    P("")
    P("## 🚨🚨 **머리 «셋» — «돌리기 «전»»에 박는다**")
    P("")
    P("```")
    ...
    # ── 자료 ────────────────────────────────────────────────────────
    CA = Path(str(r91.OUT / "184-arms.json"))   # 🔧 읽기«만»
    WARM = CA.exists()
    if WARM:
        P("## 1. 자료 — 관문")
        P("")
    ...
    def fold(t, p):
        ...
        m = min(hold + 1, len(d))
        u = [((c[k - 1] / epx * 100.0 - 100.0) if (k >= 1 and c[k - 1] is not None) else 0.0)
             for k in range(m)]
        x = [(r91.sl.net(round(o[k] / epx * 100 - 100, 2)) if o[k] is not None else 0.0)
             for k in range(m)]
        return (d[0], max(1, hold), net, u, x)

    if WARM:
        folded = [tuple(z) for z in json.loads(CA.read_text(encoding="utf-8"))]
        P("  (cache: %d trades)" % len(folded), flush=True)
    else:
        folded = [fold(t, p) for t, p in pairs]
        CA.write_text(json.dumps([list(z) for z in folded]), encoding="utf-8")
        P("  folded %d trades" % len(folded), flush=True)
    built = {"_": folded}

    # 🔧 234 — «자리»를 «달력»(모든 거래일)으로 «되돌린다»(23c:71 규약) · 원본은 «그대로» 둔다
    all_d = json.loads((Path(str(r91.OUT / "201d-arms2.json"))).read_text(encoding="utf-8"))["cal"]
    _ent = {z[0] for z in folded}
    if _ent - set(all_d):
        all_d = sorted(set(all_d) | _ent)
    pos = {d: i for i, d in enumerate(all_d)}
    n_pos = len(all_d)
    idx_at = defaultdict(list)
    for j, z in enumerate(folded):
        idx_at[pos[z[0]]].append(j)

    P("")
    P("```")
    cnt("날짜 자리 수", format(n_pos, ","), "cal — 234 가 «달력»(모든 거래일)으로 «되돌림»")
    cnt("접은 거래 수", format(len(folded), ","), "len(folded)")
    P("```")
    ...
    def place(order):
        ...
        for newp, oldp in enumerate(order):
            for j in idx_at.get(oldp, ()):
                z = folded[j]
                it[newp].append((z[1], z[2], z[3], z[4], j))
        return it

    ident = place(list(range(n_pos)))
    o, nsw0 = {}, {}
    for nm, md, sd_ in (("①", "off", 0), ("Ⓡ", "low", 0), ("Ⓓ", "rand", 7)):
        v, ns = boot_swap(ident, n_pos, md, random.Random(SEED + sd_))
        o[nm], nsw0[nm] = ann(v), ns
    PAIRS = [("Ⓡ−Ⓓ", "Ⓡ", "Ⓓ"), ("Ⓓ−①", "Ⓓ", "①"), ("Ⓡ−①", "Ⓡ", "①")]
    obsd = {nm: o[a] - o[b] for nm, a, b in PAIRS}
    P("")
    P("```")
    P("**관측(«원» 날짜축)** — ① %.2f%% · Ⓡ %.2f%% · Ⓓ %.2f%%" % (o["①"], o["Ⓡ"], o["Ⓓ"]))
    cnt("갈아탄 횟수 Ⓡ", format(nsw0["Ⓡ"], ","), "boot_swap(..., 'low')[1]")
    cnt("갈아탄 횟수 Ⓓ", format(nsw0["Ⓓ"], ","), "boot_swap(..., 'rand')[1]")
    cnt("갈아탄 횟수 ①", format(nsw0["①"], ","), "boot_swap(..., 'off')[1]  — 0 이어야 한다")
    P("   ⇒ ㉰ 관문 — ① 이 **0** 인가: %s" % ("✅" if nsw0["①"] == 0 else "🚨 **멈춘다**"))
    P("```")
    if nsw0["①"] != 0:
        return 3
    ...
    for bi in range(nboot):
        it = place(draw(n_pos, rnd))
        eqs = {}
        for nm, md, sd_ in (("①", "off", 0), ("Ⓡ", "low", 0), ("Ⓓ", "rand", 7)):
            v, _ns = boot_swap(it, n_pos, md, random.Random(SEED + sd_ + bi))
            eqs[nm] = ann(v)
        for nm, a, b in PAIRS:
    ...
    P("")
    P("```")
    P("## 🚨🚨 **한계 — 이 «기계»는 MA★ 앵커가 «없다**")
    P("")
    P("   이 «자료» 축 기계는 `sim_lots` 가 **«아니라**` `23c` 의 **`boot_eq` 규약**이다")
    P("   ⇒ **① 이 12,377만(연 +9.62%%)을 «재현하지» «않는다** — 여기선 **%.2f%%**" % o["①"])
    P("   ⇒ ★ **«상대» 비교용**이다. **«절대» 수는 «인용»하지 «않는다**")
    P("   ⚠️ `183`·`185` 도 **«같은»** 성질이다(그 판들엔 «안» 적었다 — **여기서 «적는다»**)")
    P("```")
    P("")
    P("```")
    P("## ★★ **② 읽기**")
    P("")
    for r in rows:
        P("   **%-6s** 씨앗 **%+.3f** → 자료 **%+.3f%%p** [%+.3f, %+.3f] · CI폭 **%.2f** · %s"
          % (r[0], SEEDAX[r[0]], r[1], r[2], r[3], r[6],
             "✅ **산다**" if r[5] else "🔴 **죽는다**"))
    P("")
    P("   ★ **`183` 의 가설 — 「«크기»가 아니라 «안정성»(CI 폭)이 자였다」를 «시험»한다**")
    P("     `183` 은 **다섯 점**뿐이었고 **«위쪽 끝»(−21%p)이 «없었다**")
    P("     ⇒ 이 판이 **그 «끝»을 «채운다**")
    P("")
    _big = max(rows, key=lambda r: abs(r[1]))
    _wide = max(rows, key=lambda r: r[6])
    P("   ## ⇒ 🔴 **그런데 「뒤집혔다」로 «쓰면» «과하다** — «철회»한다")
    P("")
    P("   **t = |효과| ÷ SD** 이고 — `183`·`184` 의 CI 폭이 **«비슷»**하다(12~13 · 11.5~17.5)")
    P("   ⇒ ## ★★★ **「효과가 «클수록» t 가 «크다»」는 «당연»하다 — «발견»이 «아니라» «산수»**")
    P("   ⇒ ⛔ **그러니 「`183` 의 가설이 «뒤집혔다»」도 «과하다** — **애초에 «검정»할 «가설»이 «아니었다**")
    P("")
    P("   **여덟 짝을 |효과| 순으로:**")
    _all8 = [("Ⓡ−①", 15.472, "산다"), ("Ⓓ−①", 14.721, "산다"), ("Ⓠ", 9.060, "보류"),
             ("Ⓞ3", 6.999, "죽는다"), ("Ⓞ5", 6.770, "죽는다"), ("Ⓞ10", 6.095, "죽는다"),
             ("Ⓟ", 1.678, "**산다**"), ("Ⓡ−Ⓓ", 0.751, "죽는다")]
    P("     %s" % " · ".join("%s %.3f %s" % z for z in _all8))
    P("   ⇒ 「크면 산다」가 **8 중 7** 을 «설명»하고 — **`Ⓟ`(1.678) «하나»가 «예외»**다")
    P("")
    P("## ⇒ ★★ **«정보»는 그 «예외» «하나»뿐이다**")
    P("   **Ⓟ 의 SD = 0.52** — 나머지(3.0~4.5)의 **«6분의 1»**")
    P("   ⇒ ## **«진짜» 물음: 「«무엇»이 SD 를 «작게» 만드나」**")
    P("   ⇒ ✅ 「«크거나» «좁으면» 산다」가 **«사후»** 읽기라는 것은 **«그대로»** 둔다")
    P("```")
    P("")
    P("```")
    P("## ✅ **③ 합 검산 — «맞고» «뜻»이 «강하다**")
    P("")
    _e = obsd["Ⓡ−Ⓓ"] + obsd["Ⓓ−①"] - obsd["Ⓡ−①"]
    P("   (Ⓡ−Ⓓ) + (Ⓓ−①) = %+.3f + (%+.3f) = **%+.3f** = (Ⓡ−①) %+.3f  ·  오차 **%+.4f** → %s"
      % (obsd["Ⓡ−Ⓓ"], obsd["Ⓓ−①"], obsd["Ⓡ−Ⓓ"] + obsd["Ⓓ−①"], obsd["Ⓡ−①"], _e,
         "✅" if abs(_e) < 0.01 else "🚨 **멈춘다**"))
    P("")
    P("   ## ⇒ ★★ **「«누구»를 «내보내나»는 «무의미»하고 —**")
    P("   ## **「«갈아탄다»」는 것 «자체»가 «해롭다»」**")
    P("     (`Ⓡ−Ⓓ` 가 «죽고» `Ⓓ−①` 이 «크게» 산다 ⇒ **몫이 «통째로» 「갈아타기 «자체»」에 있다**)")
    P("")
    P("## 🚨 **④ 「섞임」 — «걸린다». 그러나 판정은 «강화»된다**")
    P("")
    P("   `177:202` 「손절 «건너뛰기»가 «섞임»」 ⇒ `176:196`(「`Ⓓ−①` 은 «갈라야» 쓴다」)에 **«걸린다** ✅")
    P("   🚨 **그런데 «방향»**: 「손절 «회피»」는 **«유리»**한 쪽인데 — **«그런데도» %+.3f%%p**"
      % obsd["Ⓓ−①"])
    P("   ⇒ ★ **«섞임»이 «유리»하게 작용했는데도 «크게» 음수** ⇒ **판정 «강화»**")
    P("     (`166`·`171` 의 「치우침이 «바라는» 쪽인데도 «반대»가 나왔다」와 **«같은» 구조**)")
    P("```")
    P("")
    P("```")
    P("# 🎯 **맺음 — 「우리가 «가진» 것」**")
    P("")
    P("| «자료» 축을 통과한 것 | 무엇 | «새로» 할 것인가 |")
    P("|---|---|:--|")
    P("| **Ⓟ**(절반 익절) | 「«하지» 마라」(해롭다) | ⛔ **«하지» 말라는 것** |")
    P("| **`181㉰`** | **«회계»**(「0.5% 싸게 사면 0.5% 더 번다」) | ⛔ **«알던» 것** |")
    P("| **`177` 갈아타기** | 「«갈아타지» 마라」(합 검산으로 **«하나»**) | ⛔ **«이미» 하고 있다**(① = 자리 차면 «버림») |")
    P("")
    P("## ⇒ ★★★ **「«하지» 말라는 것 «하나» · «알던» 것 «하나» · «이미» 하는 것 «하나»」**")
    P("## ⇒ **「«새로» «할»」 것은 «0 개»**")
    P("")
    P("✅ 그건 **「«확실히» 좋다고 «말할 수» 있는 «실행 가능»한 «변경»이 «없다」」**를")
    P("   **«자료» 축에서 «확정»**한다")
    P("   ⛔ **「«없다»」가 «아니라» 「«말할 수» «없다»」** · ⛔ **「«반대»도 «말할 수» «없다»」**")
    P("")
    P("🚨 그리고 **「`184` 가 «자료» 축에서 «살아남은» «첫» 판정」은 «맞으나** —")
    P("   그게 **「«이미» 하는 것」이라 「«확인»」이지 「«얻음»」이 «아니다**")
    P("   ## ⇒ ★ **오늘 «전체»의 «모양»: «확인»은 «쌓였고» «얻음»은 «없다**")
    P("```")
    P("")
    P("```")
