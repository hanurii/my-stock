...
CACHE = Path(str(r91.OUT / ("177%s-partial.json" % ("_" + DET.replace("3C", "c").replace("PP", "p").replace("VCP", "v") if DET else ""))))
...
def main():
    ...
    P("=" * 104)
    P("")
    P("## 🔴 **머리 — 「«신호»는 «구성»에 «달렸다»」**(`179` 가 «잰» 것)")
    P("")
    P("```")
    P("🔴 **`Ⓡ−Ⓓ`(«고르는» 힘)는 «검출기 구성»에 따라 «부호»가 «바뀐다**")
    P("   원본 **+1.023** · VCP «만» **+1.679** · 3C «만» **−0.760** · (PP «만» **−0.345** · «묘사»)")
    P("   ⇒ ## **「«고르는» 힘이 «있다」」조차 «강건»하지 «않다**")
    P("")
    P("✅ **단 「«값»은 «없다」」는 «그대로»다** — `Ⓓ−① = −21.017%p` 는 **«어느» 구성에서도 «크다»**")
    P("   ⇒ ★ **이 판의 «결론**(「좋은 것을 «골라»도 «회전»이 «다» 먹는다」)**은 «살아남는다**")
    P("")
    P("⛔ 한때 「원본이 «보수적»이라 «결정» «불변»」으로 적으려 했다 — **«폐기»한다**")
    P("   3C 에서 **부호가 «반전»**되므로 「«보수적»」이 «아니다**")
    P("```")
    P("")
    if DET:
        P("")
    ...
    if abs(m1 - MA_REF) >= 1.0 and not DET:      # 🚨 `--det` 에서는 «다른» 것이 «정상»
        P("")
    ...
    if not DET:
        ...
        P("   🚨 **`--det` 판에서는 MA★ 이 «검산»을 «못» 한다** — 사본의 «정확성»은")
        P("     **«무플래그» 판(① = %s만)이 «따로» 보증한다**" % format(MA_REF, ","))
    P("")
    ...
    P("   🚨 **아래 «건수»는 «판»당이 «아니다** — Ⓡ 는 **%d판 합**, Ⓓ 는 **%d배정 × %d판 합**이다(유형 67)"
      % (n_seed, n_as, n_seed))
    for nm, dv in (("Ⓡ", n_seed), ("Ⓓ", n_seed * n_as)):
        a_, b_ = meta[nm]["skip"]
        P("   **%s** — 내보낸 %s 자리(판당 %.0f) 중 **%.1f%%** 가 ① 이라면 «그 뒤» «손절»로 끝났을 것"
          % (nm, format(b_, ","), b_ / dv, 100.0 * a_ / max(b_, 1)))
    ...
    P("")
    P("| 팔 | n(합) | **판당** | P10 | **중앙** | P90 | 음수 비율 |")
    P("|---|---:|---:|---:|---:|---:|---:|")
    for nm, dv in (("Ⓡ", n_seed), ("Ⓓ", n_seed * n_as)):
        a_ = meta[nm]["ret"]
        P("| **%s** | %s | %.0f | %+.1f%% | **%+.1f%%** | %+.1f%% | **%.1f%%** |"
          % (nm, format(len(a_), ","), len(a_) / dv, q(a_, .10), q(a_, .50), q(a_, .90),
    ...
    P("")
    P("```")
    P("## 🚨🚨 **📏폭을 «읽기» «전»에 — 「어디서」 잰 폭인가**")
    P("")
    P("   시작 자본 **%.0f만**(`account_lib.START`) 기준, «끝난» 자리:" % acc.START)
    for nm in ("①", "Ⓡ", "Ⓓ"):
        mm = st.median([a[0] for a in out[nm]])
        P("     **%s** %.0f만 — 원금 대비 **%+.1f%%**  %s"
          % (nm, mm, 100.0 * (mm / acc.START - 1.0),
             "" if mm >= acc.START else "🔴 **원금 «아래»**"))
    P("")
    P("## ⇒ 🔴 **Ⓡ·Ⓓ 는 «둘 다» 원금의 «5% 아래»로 «끝났다»**")
    P("   그런 자리에서 `P95÷P05` 가 «작은» 것은 **「«운»이 «작다»」가 «아니라**")
    P("   **「어느 씨앗으로 돌려도 «똑같이» 망했다」**는 뜻이다")
    P("   ⇒ ⛔ **「폭이 «줄어» 좋아졌다」로 «읽지» 않는다**(`175` 의 Ⓓ 와 «다른» 이유로 «작다»)")
    P("```")
    P("")
    ...
    P("```")
    P("⛔ **못 쓸 말:**")
    P("   🆕 ⛔ **Ⓡ·Ⓓ 의 «작은» 폭을 「운이 작다」로** — **«둘 다» 원금 아래로 «망했다»**는 뜻이다")
    P("   🆕 ⛔ **「내보낸 %s 자리」를 «판당»으로** — **%d판(또는 ×%d배정) «합»**이다(유형 67)"
      % (format(meta["Ⓡ"]["skip"][1], ","), n_seed, n_as))
