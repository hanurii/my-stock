...
def main():
    ...
    P("")
    P("> ## 📏 **이 문서에서 「폭」은 «항상» `P95 ÷ P05` 를 뜻한다**")
    P("> («곱셈» 자 — 27년 복리는 곱셈 과정이라 «비»가 자연스럽다)")
    P("> ⛔ **8곳을 «고치지» 않는다 — 고치면 «9번째»가 생긴다. 정의는 «여기 한 곳»에 산다**(유형 43)")
    P("")
    ...
    P("=" * 104)
    P("")
    P("| 칸 | 상한 | 위험 | 세후 총액(중앙) | 연 환산 | **씨앗 P05~P95 «백분위»**(폭 «자»가 아니다) | «세전» 낙폭 | 회복 | 매수 | 점유 |")
    P("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
    for n in SLOTS:
    ...
    # ★★ 「운의 폭」의 «자» — **P95 ÷ P05** (두뇌 세션 확정). «한 곳»에만 적는다
    SPREAD_DEF = "**운의 폭 = P95 ÷ P05**(«곱셈» 자 — 27년 복리는 곱셈 과정이라 «비»가 자연스럽다)"

    def spread(n):
        v = sorted(a[0] for a in out[n])
        return v[int(len(v) * .95)] / max(v[int(len(v) * .05)], 1)
    ...
    P("")
    P("🚨 **「운의 폭」은 «여러 자»가 있는 낱말이다 — «어느 자인지»를 «같은 줄»에 적는다:**")
    P("   " + SPREAD_DEF)
    P("")
    P("| 칸 | 자산 차(vs 칸 5) | **95% CI** | **낙폭 — 칸 5 보다** | **운의 폭 = P95÷P05** |")
    P("|---|---:|---|:--|---:|")
    res = {}
    for n in SLOTS:
        if n == 5:
    ...
    P("   ✅ **두뇌 세션이 ㉡ 로 «정했다»** — 27년 복리는 «곱셈» 과정이라 «비»가 자연스러운 자다")
    P("      (㉠ 은 «뺄셈 폭»을 «중앙»으로 나눠 **«위치»와 «흩어짐»이 «섞인다»**)")
    P("")
    P("🚨🚨 **그런데 «진짜 결함»은 «자 선택»이 «아니다»:**")
    P("   두뇌 세션은 「3.59」를, 나는 「1.22」를 썼는데 — **둘 다 «어느 자인지 «안 적고»» 썼다**")
    P("   ⇒ ★ 결함은 「자를 «잘못» 골랐다」가 아니라")
    P("     **「«자를 «안» 선언하고» 「폭」이라는 «낱말»을 썼다」**이다")
    P("   ✅ **규약: 「«폭»·«변동»·«위험»처럼 «여러 자»가 있는 낱말을 쓸 때는**")
    P("     **«어느 자인지»를 «같은 줄»에 «적는다»」**")
    P("   ★ **유형 67 로 등재됐다. 🚨 오늘 «셋»이다:**")
    P("     「폭」    P95÷P05 vs (P95−P05)÷중앙  →  두뇌 3.59 · 나 1.22")
    P("     「점유」  자리-일 vs 사건            →  146 **93.9%** vs 159 **88.3%**")
    P("     「투입률」 슬롯 점유 vs 자본 투입     →  93.9% vs **70.6%** (검증이 «두 번» 섞음)")
    P("   ★★ **검출기: 「이 낱말에 «자가 «하나뿐»»인가」를 물어라.**")
    P("     **「그렇다」고 답하면 «틀렸을 가능성이 높다»** — 측정 낱말은 «대개» 여러 자를 갖는다")
    P("     (폭 · 변동 · 위험 · 수익 · 격차 · 점유 · 노출 · 성적)")
    P("   ★ 그리고 이 처방이 **규약 ⑦ 의 «운»을 «구조»로 바꾼다** — 모든 「폭」이 «자»를 달고")
    P("     다니면 **두 사람이 «다른 자»를 쓰는 것이 «즉시» 보인다**. 「우연히」가 「«언제나»」가 된다")
    P("")
    P("🚨 **자를 ㉡ 로 바꾸면 «판정이 움직이나» — «먼저» 검사했다:**")
    P("   ㉠ 로 계산한 HD★ 통과 칸: **%s**" % (", ".join(
        "칸 %d" % n for n in SLOTS if n != 5
        and (st.median([a[1] for a in out[n]]) - st.median([a[1] for a in out[5]])) > 0.05
        and r_a[n] < r_a[5]) or "«없음»"))
    P("   ㉡ 로 계산한 HD★ 통과 칸: **%s**" % (", ".join(
        "칸 %d" % n for n in SLOTS if n != 5
        and (st.median([a[1] for a in out[n]]) - st.median([a[1] for a in out[5]])) > 0.05
        and r_b[n] < r_b[5]) or "«없음»"))
    P("   ⇒ «같다»")
    P("")
    P("🚨 **그런데 이 검사는 «질 수가 «없었다»» — 그렇게 적는다**(유형 44·63)")
    P("   두 자의 «순위»가 갈린 곳 = **칸 3↔4 «사이»뿐** · 판정선 = **칸 5**")
    P("   ⇒ ⛔ 「**확인했다**」가 아니라  →  ✅ 「**불변일 «수밖에» 없었다**」")
    P("")
    P("✅ **«질 수 «있는»» 형태로 «다시» 물었다:**")
    P("   ## **「두 자가 «판정선(칸 5) «위/아래»»를 «다르게» 부르는 칸이 «있나」**")
    n_disc = [n for n in SLOTS if n != 5 and ((r_a[n] < r_a[5]) != (r_b[n] < r_b[5]))]
    P("   ⇒ **%d 개** %s"
      % (len(n_disc), "✅ — 이건 «실패할 수 있었고», 통과했다"
         if not n_disc else "🚨 칸 %s" % ", ".join(map(str, n_disc))))
    P("   ★ 이렇게 적으면 **«둘 다» 산다** — 「앞 검사는 질 수 없었다」 «와» 「뒤 검사는 진짜 통과다」")
    P("")
    ...
    P("```")
    P("")
    # ★★ 판정 낱말은 **«함수 «하나»»**에서만 나온다 — 두 곳에 «따로» 쓰면 «어긋난다»(유형 43)
    def facts(n):
        mu, sd, lo, hi = res[n]
        spn = spread(n)
        ddn = st.median([a[1] for a in out[n]]) - st.median([a[1] for a in out[5]])
        return dict(mu=mu, lo=lo, hi=hi, spn=spn, ddn=ddn,
                    hc=(mu >= DELTA and lo > 0),          # 자산 «이김»
                    lost=(mu <= -DELTA and hi < 0),       # 자산 «짐»  (사전등록에 «없던» 칸)
                    dd_ok=(ddn > 0.05),                   # 낙폭 «얕다»
                    sp_ok=(spn < sp5))                    # 폭 «좁다»

    def verdict(n, table):
        f = facts(n)
        hd2 = f["dd_ok"] and f["sp_ok"]                   # 위험 «둘 다» 낫다
        one = f["dd_ok"] != f["sp_ok"]                    # 🆕 위험 «하나»만 나쁘다
        if table == "old":                                # 사전등록 «그대로»
            return ("둘 다 낫다" if (f["hc"] and hd2) else
                    ("**맞바꿈** ⇒ 사용자 선택" if (f["hc"] or hd2) else "바꿀 근거 없다"))
        if f["hc"] and hd2:
            return "둘 다 낫다"
        if f["lost"] and (hd2 or one):                    # 🆕 자산 «짐» + 위험이 «낫거나 반반»
            return "⛔ **선택지가 «아니다». «진» 것이다**"
        if f["hc"] and one:                               # 🆕 자산 «이김» + 위험 «하나»만 나쁨
            return "**맞바꿈** ⇒ 사용자 선택"
        if f["hc"] or hd2:
            return "**맞바꿈** ⇒ 사용자 선택"
        return "바꿀 근거 없다"
    ...
    for n in SLOTS:
        if n == 5:
            continue
        f = facts(n)
        P("칸 %-2d  자산 %s · 낙폭 %s · 폭 %s  →  **%s**"
          % (n, "✅ 이김" if f["hc"] else ("🔴 짐" if f["lost"] else "❌ 못 가림"),
             "✅ 얕다" if f["dd_ok"] else ("≈ 같다" if abs(f["ddn"]) <= 0.05 else "❌ 깊다"),
             "✅ 좁다" if f["sp_ok"] else "❌ 넓다", verdict(n, "new")))
        P("        ↳ 자산 %+.3f%%p vs Δ(%.2f)%s · 낙폭 %+.1f%%p · 폭 %.2f vs %.2f"
          % (f["mu"], DELTA, "" if f["lo"] > 0 or f["hi"] < 0 else " · CI 가 0 을 «문다»",
             f["ddn"], f["spn"], sp5))
    P("")
    ...
    for n in SLOTS:
        if n == 5:
            continue
        old, new = verdict(n, "old"), verdict(n, "new")
        ch = old != new
        if ch:
            n_flip += 1
            n_word.append(n)
    ...
    P("   칸 8   「맞바꿈 ⇒ 사용자 «선택»」 → 「«진» 것이다」 = **«선택지에서 «빠진다»»**")
    P("   칸 12  「바꿀 근거 없다」        → 「«진» 것이다」 = **낱말이 «세졌다»**")
    ...
    P("   그대로 두면 **칸 8(자산 −2.40%p = Δ 의 «두 배» 손실)**이 「위험 통과」 하나로")
    P("   **「맞바꿈 ⇒ 사용자 선택」**으로 «흘러간다». **«고르라고 내놓을 물건»이 아니다**")
    P("")
    P("🆕 **그리고 «위험 축»에도 «같은» 구멍이 있었다 — 「판정표 구멍」의 «세 번째»다:**")
    P("   161      「그 사이」에 두 경우                 →  표엔 **«한 칸»**")
    P("   162 자산축 「하나만 통과」에 두 경우           →  표엔 **«한 칸»**")
    P("   🆕 162 위험축 「둘 «다» 나쁨」의 반대인 **「«하나»만 나쁨」**  →  칸이 **«없음»**")
    P("   ⇒ 🚨 **「이번 판엔 «안» 걸렸다」는 「«다음» 판엔 걸린다」는 뜻이다**")
    P("")
    P("✅ **«새 문턱을 «안» 만들고» 채웠다 — 자산축 규칙과 «같은 모양»으로:**")
    P("   자산 «이김» + 위험 «하나»만 나쁨  →  「맞바꿈 — 사용자 «선택»」")
    P("                                      (낙폭과 폭은 **«다른 위험»**이라 «진짜» 맞바꿈이다)")
    P("   자산 «짐»   + 위험 «하나»만 나쁨  →  ⛔ 「**선택지가 «아니다». «진» 것이다**」")
    P("   ★ **낙폭 Δ·폭 Δ 를 «새로» 만들면 손잡이가 «둘» 는다. 위 규칙은 «있는» Δ 만 쓴다**")
    P("")
    P("★ 판정 낱말을 **«함수 «하나»(`verdict`)»**로 합쳤다 — 두 곳에 «따로» 쓰면 «어긋난다»")
    P("⛔ **어느 낱말을 쓸지 «내가» 정하지 «않았다»** — 위 규칙은 «검증 세션»이 준 것이다")
    P("```")
    P("")
