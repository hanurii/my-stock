...
def main():
    ...
    if jc:
        P("   ⇒ 🚨 **%d 칸이 밖이다.** ⛔ **«보정»하지 «않는다»**" % len(jc))
        P("")
        P("🚨🚨 **유형 69 — 「라벨이 «면죄부»가 된다」. «라벨»에서 «범위 좁히기»로 «바꾼다»**")
        P("   ★ «구조»다 — **«라벨»은 쓰기 «쉽고» «감도»는 «재야» 한다. 비용이 다르니 «라벨로 흐른다»**")
        P("   ✅ 가르는 규칙: **「그 미통과가 «주 판정»에 «걸리는가」를 «수»로 «보였는가」」**")
        P("")
        P("   **주 판정을 «지는» 칸의 노출 — «수»로:**")
        for vin in ("P", "R"):
            for s2 in (8.0, BASE):
                e = meta[(vin, s2)]["expo"]
                P("      %s −%-3.0f%%  노출 **%.1f%%** (현행 대비 %+.1f%%p)  %s"
                  % (vin, s2, e, e - ob, "✅ 통과" if abs(e - ob) <= 5.0 else "🚨 밖"))
        P("      ⇒ ✅ **네 칸 «전부» ±5%p 안**")
        P("      🚨 **Ⓟ −20%% 는 +%.1f%%p 로 «턱걸이» 통과다 — 그리고 이 칸은 «주 판정»에 «안» 쓴다**"
          % (meta[("P", 20.0)]["expo"] - ob))
        P("        (「턱걸이」가 «변명»이 아니라 **«범위 문장»**이 되는 자리)")
        P("      ★ Ⓡ 의 −6·−8 은 **위험항이 아니라 «상한»이 물어** Ⓟ 와 **«같은 포지션»**이다")
        P("        ⇒ 그래서 **−8%·−10% 는 두 벌에서 «같은 수»**이고 **노출도 «같다»**")
        P("")
        P("   ⇒ ✅ **「−10% 가 최선」·「−8% 와 «구분되지 않는다」 두 문장은 «미통과 칸에 «안» 걸린다»**")
        P("   ⇒ 🚨 **걸리는 건 「넓힐수록 나빠진다」 «하나»**이고, 거기서 —")
        P("")
        P("🚨🚨 **「노출」 문장에 «어느 팔»인지가 «없으면» 두 수가 «부딪히는» 것처럼 보인다**(유형 67 의 «팔» 판)")
        P("   **Ⓡ** −12/−15/−20 노출 **%.1f / %.1f / %.1f%%**  →  «떨어진다»(포지션이 «작아져서»)"
          % tuple(meta[("R", z)]["expo"] for z in (12.0, 15.0, 20.0)))
        P("   **Ⓟ** −12/−15/−20 노출 **%.1f / %.1f / %.1f%%**  →  «올라간다»(«덜» 털려서)"
          % tuple(meta[("P", z)]["expo"] for z in (12.0, 15.0, 20.0)))
        P("   ⇒ **«부딪히는» 게 아니라 «다른 팔»**이다. **문장마다 «팔»을 붙인다**")
        P("")
        P("★★★ **그리고 그게 「남은 물음」을 «절반» 닫는다:**")
        P("")
        P("   | 팔 | 노출 | 연 환산 |")
        P("   |---|---:|---:|")
        for v_, z in (("P", BASE), ("P", 20.0)):
            P("   | %s −%.0f%%%s | **%.1f%%** | **%+.2f%%** |"
              % (v_, z, " «현행»" if z == BASE else "", meta[(v_, z)]["expo"],
                 acc.cagr(st.median([a2[0] for a2 in out[(v_, z)]]), YRS)))
        P("")
        P("   ⇒ 🚨 **Ⓟ −20%% 는 노출이 «더 높은데»(%+.1f%%p) 성적은 «절반 이하»다**"
          % (meta[("P", 20.0)]["expo"] - ob))
        P("   ## ⇒ ✅ **「돈이 «놀아서»」가 «아니다». Ⓟ 에서는 «갈렸다» — «현금 탓»이 «아니다»**")
        P("   ## ⇒ 🚨 **«Ⓡ 에서만» 못 갈랐다**")
        P("   ★ **이게 「두 벌을 «다» 내라」의 «보상»이다.** 한 벌만 냈으면 «영영» 못 갈랐다")
    else:
        P("   ⇒ ✅ **전부 ±5%p 안**")
    P("")
    ...
    P("")
    P("★★★ **JD★ 를 «상시 관문»으로 «승격»한다** — 이 판이 «근거»다")
    P("   🚨 첫 구현은 중복제거 `open_until` 을 **연도를 넘겨** 유지했다(162·161·156 은 **«해마다» 초기화**)")
    P("      ⇒ **11,713만 vs 12,377만** — 관문이 «없었으면» 12개 팔이 «전부 조용히» 어긋났다")
    P("")
    P("   ✅ **「모든 새 판은 «현행 규칙 칸»을 «포함»하고, 그 칸이 «앞 판의 수»와 «일치»해야 한다」**")
    P("   ✅ **«공짜»다** — 현행 칸은 «어차피» 비교 기준으로 필요하다")
    P("      (156·161·162·163 이 «전부» **12,377만** · **163 에서만 «안 맞아» 결함이 나왔다**)")
    P("   🚨 **한계 — «같은 줄»에**: **«세대가 «같을» 때»만 «일치»를 요구한다.**")
    ...
    P("")
    P("★★ **절단은 «한 방향»이다 — 「강화」로 «쓴다»:**")
    P("   창이 끝나 «아직 회복 중»이면 **«진짜 회복 ≥ 관측값»**이다. **«짧을 수는 «없다»»**")
    P("   ⇒ 「13.5년」은 **«하한»**이고, 동점을 «절단 아님»으로 셌으니(`>` 비교)")
    P("     **24/60 은 «하한의 하한»**이다 ⇒ **「−20%는 나쁘다」를 «약화»가 아니라 «강화»**한다")
    P("```", flush=True)
    ...
    P("")
    P("★★ **«두 줄»을 «다» 적어야 «완전»하다 — 하나만 쓰면 «회피»거나 «원인 미상»이다:**")
    P("   ① 🔴 **「예측이 «틀렸다»」**                                         ← «정직»")
    P("      (「차이가 «커질» 것」은 **«부호가 같다»를 «전제»**한다. 부호 반전이 그 전제를 «무너뜨렸다»)")
    P("   ② ⛔ **「«왜» 틀렸는지는 «못 가른다»」** — `68` 과 **«두 축»**이 «같이» 다르다  ← «범위»")
    P("   ⇒ **①만이면 «원인 미상» · ②만이면 «회피» · «둘 다»가 «완전한 서술»**")
    P("")
    ...
    P("")
    P("🚨 **라벨의 «두 종류»를 가른다 — 없으면 유형 69 가 「라벨을 «쓰지 마라」」로 «과잉교정»된다:**")
    P("   ⛔ **«면죄부» 라벨** = 판정에 **«쓰면서»** 감도를 «안» 잰 것   ← **이 판에서 «없앴다»**")
    P("   ✅ **«안 쓴다» 라벨** = 「최악 씨앗」처럼 판정에 **«안»** 쓰는 것 ← 그대로 둔다")
    P("   ★ 이 단서는 «이미 있는» 시험(「그 미통과가 «주 판정»에 걸리는가」)의 **«거울»**이다")
