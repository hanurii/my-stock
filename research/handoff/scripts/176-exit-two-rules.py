...
def main():
    ...
    def gain_at_sig(n):
        ...
        for t, p in pairs:
            ...
            if jx is None:
                continue
            out_.append(p["c"][jx - 1] / t["entry_px"] * 100.0 - 100.0)
        return sorted(out_)

    # ═══ UA★ — «세기» «전»에 «코드»로 ══════════════════════════════════
    P("")
    P("## 🚨🚨 **한정 — 「«수익 중»」을 «어떻게» 구현했나. «말» 아닌 «수»로**")
    P("")
    P("```")
    P("🔎 `sig_reversal()`:  `if c[j] is None or **c[j] <= epx**: continue`")
    P("")
    P("## 🔴 **「수익 중」 = 「종가가 진입가보다 «1원»이라도 «위»」 · «비용 «전»»이다**")
    P("   원전은 **「«괜찮은» 수익」**이라 했다 — **«같은 자»가 «아니다**")
    P("")
    P("**ⓐ 가 «켜진» 그날의 «수익률» 분포**(그 문턱이 «얼마나» 낮았나):")
    P("")
    P("| N | 켜진 건수 | P10 | **중앙** | P90 | **+5% 미만** | **+10% 미만** |")
    P("|---|---:|---:|---:|---:|---:|---:|")
    for n in NS:
        g = gain_at_sig(n)
        if not g:
            continue
        P("| **%d** | %s | %+.1f%% | **%+.1f%%** | %+.1f%% | **%.1f%%** | **%.1f%%** |"
          % (n, format(len(g), ","), g[int(len(g) * .10)], g[int(len(g) * .50)],
             g[int(len(g) * .90)],
             100.0 * sum(1 for x in g if x < 5.0) / len(g),
             100.0 * sum(1 for x in g if x < 10.0) / len(g)))
    P("")
    P("## ⇒ ★ **문턱이 «낮을수록» 「반전 매도」가 «더 «자주»» 발동한다 ⇒ 우리는 «원전보다» «자주» 팔았다**")
    P("")
    P("🚨 그리고 이 판이 **「«늦게» 팔수록 «덜» 나쁘다(단조)」**를 냈으니 —")
    P("   **「문턱을 «올렸으면» «덜» 나빴을 수 «있다»」** ⇒ ⛔ **«안» 쟀다. «등록»만 한다**")
    P("")
    P("## ⇒ ⛔ **「«원전»이 «틀렸다»」로 «닫지» 않는다 — 「«우리» 구현에서는 «해로웠다»」까지다**")
    P("```")
    P("")
    ...
    P("     ⇒ 🔴 **`Ⓓ−①` 은 「«못» 쓴다」가 «아니라» — 「**«섞여» 있어 «갈라야» 쓴다**」")
    P("       (「일찍 판다」 + 「손절을 «안» 맞는다」 ⇒ **«가르는» 것은 «다른 판»의 일**)")
    ...
    P("   🆕 ⛔ 「**«원전»이 «틀렸다**」 — **「«우리» 구현에서는 «해로웠다»」**까지다(위 «한정»)")
    P("```")
    P("")
    P("```")
    P("★ **«기록» — 이 판이 «남기는» 것 «셋»**")
    P("")
    P("   ㉠ ## **이 판은 「«손절» «전»」의 세계를 잰다**")
    P("     조기 청산은 ① 의 «손절 32~52%»를 «통째로» 없앤다 ⇒ **«비교»가 그 위에서 일어난다**")
    P("")
    P("   ㉡ ## **검증이 «미리» 준비한 «퇴로»가 «필요 «없었다»»**")
    P("     「1.4배 요구가 «모든» 경로에 걸리면 0 건 — 그래도 «결과»다」로 «퇴로»를 준비했는데")
    P("     **«코드»를 «먼저» 읽으니 «항등식»이 «아니었다»** ⇒ 퇴로를 «안» 썼다")
    P("     ★ **「쓸 수 «있을지»를 «코드»로 «먼저» 본」 것이 그 준비를 «불필요»하게 만들었다**")
    P("")
    P("   ㉢ ## **「없다」를 «두 세션»이 «따로» 읽었다**(유형 35 의 «집행»)")
    P("     룩어헤드 — `sig_supply` `range(1,…)`+`return j+1` · `sig_reversal` `range(0,…)`인데")
    P("     `return j+1` 이고 `low_win` 이 **«그날»을 «뺀다»** ⇒ **세 곳 «전부» 깨끗**")
    P("```")
    P("")
    P("```")
