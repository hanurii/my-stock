...
def main():
    ...
    P("")
    P("## 1. 🔴🔴 **«라벨»이 «틀렸다» — 「적자」가 «지금»이 «아니라» «1년 전»이다**")
    P("")
    P("```")
    P("🔎 명령 `sed -n '54,58p' 103-code33-strength.py` · **내가 «직접» 읽어 확인했다**")
    P("")
    P("    e0 = _yoy(g(q,   \"eps\"), g(**q−4**, \"eps\"))   ← prev = **「«1년 전» «그» 분기」**")
    P("    e1 = _yoy(g(q−1, \"eps\"), g(**q−5**, \"eps\"))   ← prev = **「«1년 전»의 «그 직전» 분기」**")
    P("")
    P("# 🔴 **N2(ii) = 「«1년 전» «그» 분기의 EPS ≤ 0」이다**")
    P("   **「«지금» 적자」가 «아니고** · **「회사가 «적자 상태»」도 «아니다**")
    P("## ⇒ ★★ **「«1년 전»에 적자였던 것을 «지금» 산다」 = «턴어라운드» 자리다**")
    P("")
    P("✅ **그리고 물음 ① 의 답은 «둘 «다»» 참이다 — «두 문장»을 «같이» 적는다:**")
    P("   ① **「«우리 계산식»이 «못» 재는 것」**이다(`_yoy` 가 `prev<=0` 을 «버린다»)")
    P("   ② 그리고 그 «못 재는» 것이 **«마침» 「«1년 전» 적자」를 «고른다»**")
    P("")
    P("🆕 ✅ **「«지금»(q 분기) EPS 가 «양수»인가」를 «별도 열»로 찍는다 — «턴어라운드»인가 «계속»인가:**")
    tot = max(cur_neg + cur_pos + cur_na, 1)
    P("")
    P("| «지금»(q) EPS | 뜻 | n | N2(ii) 중 |")
    P("|---|---|---:|---:|")
    P("| **> 0** | ## **«턴어라운드»**(1년 전 적자 → 지금 «흑자») | **%s** | **%.1f%%** |"
      % (format(cur_pos, ","), 100.0 * cur_pos / tot))
    P("| **≤ 0** | **«계속» 적자** | **%s** | **%.1f%%** |"
      % (format(cur_neg, ","), 100.0 * cur_neg / tot))
    P("| «없음/NaN» | — | %s | %.1f%% |" % (format(cur_na, ","), 100.0 * cur_na / tot))
    P("")
    P("## ⇒ ★★ **한 열로 «갈린다» — Ⓚ 는 «두 무리»가 «섞인» 팔이다**")
    P("   **«턴어라운드» %.1f%% + «계속 적자» %.1f%%**"
      % (100.0 * cur_pos / tot, 100.0 * cur_neg / tot))
    P("   ⇒ 🚨 **Ⓚ 가 «이겨도» «어느 무리» 때문인지 «이 판»은 «못» 가른다** — **«다음 판»의 물음**이다")
    P("")
    P("   ⛔ **팔 이름을 「«적자»」로 «쓰지» 않는다** — **오도**다")
    P("   ✅ 정확한 이름: **「«1년 전» «그» 분기가 «적자»였던 것」**")
    P("")
    ...
    P("")
    P("🆕 🚨 **「합 검산이 «없다»」로 «안» 적는다 — 「없다」로 적으면 그게 «유형 35»다**")
    P("   ✅ **「겹치지 «않는다»」 «자체»가 «항등식»**이다. 그래서 «다른» 관문을 «건다»:")
    P("     ㉠ **Ⓚ ∩ Ⓛ 겹침 = «%d»**  →  %s"
      % (len(K & L), "✅ **0**" if not (K & L) else "🚨 **멈춘다**"))
    P("     ㉡ **Ⓚ + Ⓛ = %s ≤ ① %s**  →  %s"
      % (format(len(K) + len(L), ","), format(len(kept), ","),
         "✅" if len(K) + len(L) <= len(kept) else "🚨 **멈춘다**"))
    P("   ★ **「«덧셈»」이 «없으면» 「«교집합»」과 「«포함»」을 «건다». 관문은 «분해»에만 있는 게 «아니다**")
    P("")
    ...
    P("")
    P("```")
    P("🆕 🚨 **SB★ — 「«축»의 실패」와 「«자리» 벽」을 «가르는» 수**")
    P("   앞 «열한» 판의 공통 결론: 「막힌 곳은 «고르는 «능력»»이 아니라 **«고를 «자리»»**」")
    P("   ⇒ **173 이 «져도» 「«축»이 나쁘다」인지 「«자리»가 없다」인지 «못» 가린다**")
    P("")
    P("| 팔 | 살 수 «있었던» 수 | «실제로» 산 수 | **체결률** |")
    P("|---|---:|---:|---:|")
    for nm in ("①", "Ⓚ", "Ⓛ"):
        P("| **%s** | %s | %.0f | **%.1f%%** |"
          % (nm, format(n_pre[nm], ","), meta[nm]["n"],
             100.0 * meta[nm]["n"] / max(n_pre[nm], 1)))
    P("")
    P("   자리 **%d 칸** · Ⓚ 후보 **%s** ⇒ **자리가 «구조적»으로 «모자란다»**"
      % (SLOTS, format(n_pre["Ⓚ"], ",")))
    P("   ⇒ ✅ **체결률이 «작으면» 그건 «자리» 벽이고 «축»의 실패가 «아니다»**")
    P("   ⇒ ★ **판정 «낱말»이 «달라진다** — 「축이 나쁘다」가 «아니라» 「«자리»가 없어 «못 잰다»」")
    fr = [100.0 * meta[n]["n"] / max(n_pre[n], 1) for n in ("①", "Ⓚ", "Ⓛ")]
    P("")
    P("## ⇒ ★★ **체결률이 «세 팔» «거의 같다**(%.1f / %.1f / %.1f%%)"
      % (fr[0], fr[1], fr[2]))
    if max(fr) - min(fr) < 3.0:
        P("   ⇒ ✅ **«자리» 벽은 «있으나» — **«팔마다» «같다»** ⇒ **Ⓚ−Ⓓ 의 «차»를 «오염»시키지 «않는다»**")
        P("   ⇒ ★ 그래서 **「«자리»가 없어 «못 잰다»」로 «빠져나갈» 수 «없다»** — «축»을 «잰» 것이 맞다")
        P("   🚨 **단 「자리가 «있었으면» «어땠을지»」는 «여전히» «안» 쟀다**(모두 %.0f%%만 산다)"
          % fr[0])
    else:
        P("   ⇒ 🚨 **체결률이 «갈린다»(%.1f%%p) — 「«자리» 벽」이 팔마다 «다르게» 걸린다**"
          % (max(fr) - min(fr)))
    P("```")
    P("")
    P("```")
    ...
    P("| 짝 | 차 | **95% CI** | 📏폭 | 판정칸 |")
    P("|---|---:|---|---:|:--|")
    for lab, x, y in (("**Ⓚ−Ⓓ** 「1년 전 그 분기 적자」를 골라 삼 ★**판정**", "Ⓚ", "Ⓓ"),
                      ("Ⓛ−Ⓔ «가속»만 삼 — 🚨 **«수»만. 판정칸 «없음»**", "Ⓛ", "Ⓔ"),
                      ("Ⓚ−① «참고» — 🚨 **«수»만. 판정칸 «없음»**", "Ⓚ", "①")):
        mu, lo, hi = dif(x, y)
        ...
        if lo >= DELTA:
            ...
            vd = "🚨 **못 가린다**"
        if x != "Ⓚ" or y != "Ⓓ":
            vd = "—"                      # 🚨 «구조»로: 판정 짝이 «아니면» 칸을 «안» 붙인다
        elif sx > sy * 1.2 and "판정칸 1" not in vd:
            vd += " · 🚨 **📏폭 %.2f → %.2f 로 «크게» 넓어짐**" % (sy, sx)
        P("| %s | **%+.3f%%p** | [%+.3f, %+.3f] | %.2f vs %.2f | %s |"
          % (lab, mu, lo, hi, sx, sy, vd))
    P("")
    P("```")
    P("🚨 **판정은 «Ⓚ−Ⓓ» «하나»다** — **Ⓛ 이 «이겨도» «결론»으로 «안» 쓴다**")
    P("   ⇒ Ⓛ 이 이기면 그건 **«다음 판»의 «물음»**이지 **«이 판»의 «답»이 «아니다**")
    P("   ✅ 그래서 **위 표가 Ⓛ−Ⓔ 에 «판정칸»을 «안» 붙인다** — «수»만 «인쇄»한다(«구조»로)")
    P("```")
    ...
    P("   ⛔ **노출 «보정»**(유형 73)")
    P("   ⛔ **팔 이름을 「«적자»」로** — **「«전년동기» 분기가 «적자»」**다(위 §1)")
    P("   🆕 ⛔ **「Ⓚ−Ⓓ 가 Ⓛ−Ⓔ «보다» 크다」** — 각자 «자기» 플라세보와만 맞댔다.")
    P("     **«서로» 맞대는 «순간» «고르기»가 된다** ⇒ **«구조»로 막았다**(위 표가 판정칸을 «안» 붙인다)")
    P("```")
    P("")
    P("```")
