...
def main():
    ...
    P("         ⇒ **%s**" % ("✅ **통과**" if (mu >= DELTA and lo > 0) else
                             "🚨 **미통과**"))
    m3 = st_["③−⑤″ (CD★ 음성 · 순열)"][0]
    lo3, hi3 = st_["③−⑤″ (CD★ 음성 · 순열)"][2:]
    P("**CD★**  ③−⑤″ = **%+.3f%%p**  CI [%+.3f, %+.3f]  →  %s"
      % (m3, lo3, hi3, "🚨 **양수 — CC 를 «무효»로 읽는다**" if lo3 > 0
         else "✅ CC 를 «무효»로 만들지 않는다"))
    if lo3 <= 0 <= hi3:
        P("         🚨 **CI 가 0 을 «포함»한다(상단 %+.3f — «겨우» 포함).**" % hi3)
        P("         ⛔ **「③ 은 해롭지 «않다»」로 «읽지 않는다». «못 가린다»가 맞는 낱말이다**")
    P("")
    P("### ★★ ②와 ③의 **«비대칭»** — 주변분포를 맞추니 «갈렸다»")
    P("")
    P("```")
    P("② (상위1/3 을 **낮춤** +30→+20)  순열보다 **%+.3f%%p** · **0 «배제»**  →  **해롭다**"
      % st_["**②−⑤′ (CC★ 주판정 · 순열)**"][0])
    P("③ (상위1/3 을 **올림** +30→+40)  순열보다 **%+.3f%%p** · **0 «포함»**  →  **못 가린다**" % m3)
    P("")
    P("🚨 **«옛» 플라세보(균등 1/3)에서는 둘이 «비슷»했다** — 그때는 「방향과 «무관»하게 해롭다」로 읽혔다.")
    P("   **주변분포를 맞춘 순열 플라세보에서 «갈렸다».** ⇒ 그 읽기는 **«철회»된다**")
    P("★ 「더 나은 대조군 «하나»가 강한 문장을 깬다」 — 이 판의 소득이다")
    P("```")
    P("```")
    P("")
