...
def main():  # noqa: C901
    ...
    P("⛔ 🆕 ★ **«전량»(`HALF = 1.0`) 매도는 «여전히» «안** 쟀다")
    P("     📄 `strategy_params.py:18-19` 이 «스스로» 적어 뒀다: 「위 근거는 «전부» "
      "「＋30 에 «절반» 팔고 나머지는 «추격»」(`HALF=0.5`)이다. **「＋30 «전량» 매도」는 «잰 적이 «없다»**」")
    P("     🚨 **`256` 은 `HALF` 를 0.5 «주변»(1/3·0.75)으로«만** 돌렸다 ⇒ **1.0 은 «안** 돌렸다")
    P("     ⛔ **그러니 「`256` 이 «비율» 축을 «쟀다»」가 「«비율»은 «다» 봤다」로 «읽히면» «안** 된다")
    P("        (`131:82` 이 **「1.0 은 «우리» 것」**이라 적어 뒀으니 — **«원전» 축이 «아니다**)")
