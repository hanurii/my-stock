...
def main():
    ...
    for wnm, y0, y1, d0, d1 in (("전체 27.4년", 2000, 2025, D0, D1),
        ...
        for tk in ("SPY", "QQQ"):
            ...
            t = T95.get(n, 2.06)
            lo, hi = m - t * se, m + t * se
            geo = 100.0 * (math.prod(1.0 + v / 100.0 for v in ex) ** (1.0 / n) - 1.0)
            zero = lo <= 0 <= hi
            print("")
            print("### 우리 − %s   (연 관측 **%d개**)   %s"
                  % (tk, n, "🏆 «이기는» 비교" if tk == "SPY" else "🔻 «지는» 비교"))
            print("```")
            print("연 초과수익  산술 **%+.2f%%p**  ·  기하 **%+.2f%%p**  ·  **SD %.2f%%p**" % (m, geo, sd))
            ...
            print("🚨 **「기하」는 «차이 계열»을 복리한 값이지 «복리 격차»가 «아니다»** —")
            print("   경제적 의미의 복리 격차는 아래 «분해» 절의 **세전 −0.05%p / 세후 −1.23%p** 다.")
            print("   ⇒ CI 는 **«산술»에 붙었다**. 기하는 «변동성 드래그»를 보여 주는 참고값일 뿐")
            print("```")
            ...
            out["%s|%s" % (wnm, tk)] = {"n": n, "mean": m, "geo": geo, "sd": sd, "ci": [lo, hi],
    ...
    print("```")
    print("**세전**(이 판)   우리 중앙 %.2f배 = 연 **%+.2f%%**  ·  QQQ %.2f배 = 연 **%+.2f%%**"
          % (med, cagr(med), idx["QQQ"][1][-1], cagr(idx["QQQ"][1][-1])))
    print("                  →  격차 **%+.2f%%p**" % (cagr(med) - cagr(idx["QQQ"][1][-1])))
    print("**세후**(129·150) 우리 9,280만 = 연 **+8.47%**  ·  QQQ 12,627만 = 연 **+9.70%**")
    print("                  →  격차 **-1.23%p**")
    print("")
    print("⇒ ★ **격차의 «대부분»이 «세금»에서 온다** — 우리는 27년간 **644번** 팔아 해마다 실현하고,")
    print("   QQQ 그냥 보유는 **1번** 판다(27년 이연). 같은 세율이어도 «내는 시점»이 다르다")
    print("⚠️ 그리고 이 세전 비교는 **우리에게 «불리»**하다 — 우리는 배당이 «빠졌고» QQQ 는 «들었다»")
    print("```")
    print("> ### 🚨 **그래도 «못 가린다»는 안 바뀐다** — 위 CI 가 ±13%p 폭이고 격차는 1%p 자리다")
    print("> ### ⛔ 그리고 이건 **「세금만 없으면 이긴다」가 «아니다»** — 세금은 «실제로 낸다»")
