...
# 🚨 2026-08-30 — **손절을 −8 에서 −10 으로 바꿨다.**
#    이유 셋이 «독립적으로» 같은 값을 가리켰다:
#      ① 프로젝트 매매 정본 `scripts/canslim_lib/strategy_params.py:11` = 10.0
#      ② 사용자님 실전 규칙 (2026-08-13 부터 +20/−10)
#      ③ 원전 — 승률 40% 대에서 최적 비율이 20/10 (126b 에서 우리 자료로 확인)
#    −8 은 74→81→91 계보에서 «비교하려고» 얼려 둔 값이었고 정본과 갈라져 있었다.
# ⛔ **이 줄 위쪽에서 나온 옛 결과(74 +298.44% · 81 · 91 표본밖 판정)는 −8 로 잰 값이다.**
#    그대로 재현하려면 `--legacy-stop8` 을 준다.
STOP, TARGET = 10.0, 20.0
if "--legacy-stop8" in sys.argv:
    STOP = 8.0
LO, HI = 0.10, 0.30
N_SEED = 200
