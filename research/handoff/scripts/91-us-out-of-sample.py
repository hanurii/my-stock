...
# 🚨 2026-08-30 — **손절은 −8 이다. 이번엔 «표류»가 아니라 «결정»이다.**
#
#  ▸ 오전: −8 이 74→81→91 계보에서 «비교하려고» 얼려 둔 값이고 매매 정본(−10)과
#         갈라져 있던 걸 발견 → 정본·사용자 실전·원전이 모두 −10 을 가리켜 10.0 으로 바꿨다.
#  ▸ 오후: 128 이 **사용자님이 세운 자**로 다시 줄 세웠다 —
#         「bad case 일 때 손실을 최대한 막으면서 수익률은 최대한 끌어올린다」
#         창 8,800개(판 20 × 시작일)에서 **네 기간 × 네 항목 16칸 중 −8 이 15칸 승**
#         (하위5% · 중앙 · 낙폭 · 원금손실률 전부. −10 이 이긴 건 4년 «최악» 한 점)
#  ▸ **사용자 결정(2026-08-30): −8 로 간다.**
#
# ⛔ 남는 어긋남 — **이건 «총액 기준»에서는 지는 선택이다. 알고 고른 것이다**
#    27.4년 총액   −8 14,382만  <  −10 15,644만
#    원전의 표(10회 거래 «복리»)도 «총액» 기준이라 −10 을 가리킨다.
#    **자가 다르면 답이 다르고, 사용자님은 «창 기준»을 골랐다.**
# ⛔ 그리고 **매매 정본 `strategy_params.py:11` 은 아직 10.0 이다** — 실전 규칙은 별개 결정이다.
STOP, TARGET = 8.0, 20.0
if "--stop10" in sys.argv:
    STOP = 10.0
LO, HI = 0.10, 0.30
N_SEED = 200
