...
🚨🚨 2026-09-02 — **계좌를 만드는 자리는 이제 `account_lib.py` 하나다**
------------------------------------------------------------------
「지수 숏」은 **여기(91)에 «없었고»**, 스크립트 **18개**가 각자 자기 `account()` 안에
«베껴» 갖고 있었다 ⇒ **「빼자」는 결정을 내려도 «뺄 자리»가 없었다.**
```
✅ 사용자 결정 — **지수 숏을 «뺀다»**  (원전 근거 없음 · 우리 발명 · 차입료 확정 지출 ·
                                        150 비교에서 «우리 쪽에만» 얹혀 있어 불공정)
✅ 새 판은 **`account_lib.account(x)`** 만 쓴다 — 그 파일은 108 을 **«import조차» 안 한다**
⛔ 117~154 의 이미 «보고된» 결과는 **숏이 얹힌 채다. 안 고친다** (문서–코드 동기)
   ⇒ **세대가 갈렸다. 섞어 쓰지 말 것.**
```
"""
...
# ★ 2026-08-30 최종 — **손절 −10. 사용자 결정이고, «자를 고른» 결정이다.**
#
# 하루에 값이 세 번 움직였으므로 경위를 전부 남긴다.
#  ▸ 발견  −8 은 74→81→91 계보에서 «비교하려고» 얼려 둔 값이었고 매매 정본(−10)과 갈라져 있었다
#  ▸ 128   **사용자님 자**(「bad case 손실 최소 ∧ 수익 최대」)로 창 8,800개를 줄 세우니
#          네 기간 × 네 항목 16칸 중 **−8 이 15칸 승** (하위5%·중앙·낙폭·원금손실률)
#  ▸ 반면  **27.4년 «총액»은 −10 이 이긴다** (15,644만 vs 14,382만 = **+8.8%**)
#          원전의 표(10회 거래 «복리») 도 «총액» 기준이라 −10 을 가리킨다
#  ▸ 결정  사용자님: 「저는 주식투자를 **평생** 할 계획입니다. 돈을 중간에 빼더라도
#          **다 빼진 않을** 테니 **길게 보면 −10 이 더 나은 수익**을 가져다주지 않겠습니까」
#          → **자를 「창」이 아니라 「총액」으로 고른 것이고, 그 전제에서 −10 이 맞다.**
#
# ⛔ 알고 치르는 값 — **짧은 구간의 «나쁠 때»가 더 나쁘다. 그건 받아들인 비용이다**
#    3년 하위5%    −10 884만  vs  −8 953만
#    3년 원금손실  −10 15.4%  vs  −8 7.6%
#    5년 낙폭 하위5%  −10 −38.9%  vs  −8 −34.8%
# 🚨 남은 어긋남: **「평생 굴리며 중간에 «일부»만 뺀다」는 경로는 «둘 중 어느 자도 아니다».**
#    총액 기준(=한 번도 안 뺀다)과 창 기준(=다 빼서 쓴다) «사이»다. 아직 «안 쟀다».
# ★ 2026-09-02 — 목표 **+20 → +30** (사용자 결정). 154: 세금 «전»에도 48/60 (p=1.6e-06)
#   ⛔ 「+30 이 지수를 이긴다」는 «아니다» — 152 가 「못 가린다」로 닫았다
#   🚨 근거는 전부 **HALF=0.5(절반+추격)** 위의 것이다. 「전량 매도」는 «안 쟀다»
STOP, TARGET = 10.0, 30.0
