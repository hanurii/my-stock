...
def main():
    ...
    P("")
    P("🚨 **«범위» — 이 기저율은 «우리 후보 «안»»의 것이지 «시장 전체»가 «아니다**")
    P("   (후보는 «이미» 추세 관문을 통과한 것들이다) ⇒ **원전이 말한 모집단과 «다를 수» 있다**")
    P("   ⇒ ✅ 그래도 **「같은 모집단 «안»에서 대박군이 «더» 높은가」**는 «답할 수» 있다 — 그게 이 판이다")
    P("```", flush=True)
    ...
    for x, y in (("①", "Ⓓ"), ("Ⓘ", "Ⓓ")):
        ...
        obs = dif(x, y)[0]
        P("   %s−%s 노출 차 **%+.1f%%p** → 보정 **%+.3f → %+.3f%%p**"
          % (x, y, ex - ey, obs, obs - slope * (ex - ey)))
    P("```")
    ...
    P("   ⛔ 「원전 ①이 맞다/틀리다」 — **«좋은 실적»의 자가 «우리» 것**이다          ← «정의». 손글씨")
