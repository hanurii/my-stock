...
- 피벗 후보: `sepa-vcp-candidates.json` 과 `sepa-power-play-candidates.json` 의
  `pivot_price` (vcp 우선 순서). 매수일 부근에서 **실제 돌파가 확인되는** 피벗을
...
| 6 | squat | 스쿼트 (돌파 실패) | 돌파일 다음 날부터: 종가가 피벗 아래로 되돌아온 날이 있으면 위반. 피벗이 없으면 na. 피벗 돌파가 확인되지 않았으면(돌파일 추정) na — 돌파 없이는 돌파 실패도 없음. |
