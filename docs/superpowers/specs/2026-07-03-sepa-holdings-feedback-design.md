...
> ## 🚨 2026-09-10 정정 — `early_sell` 은 **`needs_review`(🟡 점검)로 «내려갔다»**
>
> **사용자 결정.** 이 문서가 정의한 「위반 «하나»면 «조기매도»」 문턱을 **미국 27.4해(Sharadar)로 재니**
> **계좌가 «무너졌다» — 해당 −17.955%p**(규칙 없음 +10.160%p/해 → 규칙 켬 −7.795%p/해 · 칸 2 · 네 칸 «전부»).
> 문턱을 «다섯»까지 올려도 +6.712 로 규칙 없음을 «못» 넘었다.
> ⇒ **신호는 «지우지» 않고 「매도」에서 「«점검»」으로 내렸다.** 자동 매도는 «트레일링»이 맡는다.
> ⛔ **「매도 규칙이 «쓸모없다»」가 «아니다»** — 「어느 규칙이 나쁜 종목을 짚는가」는 **«못» 가렸다**(칸 5).
> 🔎 근거·판정 전문: `sell_rules.py` 의 `needs_review` 주석 · `results/253`·`254`·`255`
...
```json
{
  ...
  "holdings": [
    ...
    { "code": "271560", "name": "오리온",         "buy_datetime": "2026-07-02 09:07:53", "buy_price": 138500, "quantity": 72, "pivot_price": 138090 },
...
- 종목별 `pivot_price` 를 적으면 **매수 시점 피벗 스냅샷**으로 최우선 사용한다
  (`pivot_source: "manual"`). 이후 스캔에서 후보 파일의 피벗이 바뀌거나 사라져도
  "내가 산 근거"로 일관되게 판정하기 위한 장치. 매수 때 화면의 피벗을 적어두는 것을 권장.
...
- 피벗 후보: ① 매수 목록의 `pivot_price` 스냅샷이 있으면 **최우선**(manual).
  ② 없으면 `sepa-vcp-candidates.json` 과 `sepa-power-play-candidates.json` 의
  `pivot_price` (vcp 우선 순서) 중 매수일 부근에서 **실제 돌파가 확인되는** 피벗을
  먼저 선택하고, 어느 것도 돌파 확인이 안 되면 첫 후보(vcp 우선)를 쓰되
  돌파일은 매수일로 추정한다.
- 돌파일: 매수일로부터 거슬러 최대 20거래일 안에서, "전일 고가 ≤ 피벗 < 당일 고가"
  인 가장 최근 날 — **장중 돌파를 인정**한다(종가가 피벗 아래여도 장중에 넘었으면 돌파일).
  못 찾으면(피벗 없음 포함) **매수일을 기준점으로 대체**하고
...
| 6 | squat | 스쿼트 (돌파 실패) | **돌파 당일부터**: 종가가 피벗 아래로 되돌아온 날이 있으면 위반 — 장중 돌파 후 당일 아래 마감이면 "돌파 당일 스쿼트". 피벗이 없으면 na. 피벗 돌파가 확인되지 않았으면(돌파일 추정) na — 돌파 없이는 돌파 실패도 없음. |

돌파일 다음 날 데이터가 아직 없으면(오늘 돌파·매수) 2~5번은 pending
(스쿼트는 돌파 당일 종가로 즉시 판정 가능).
