...
| `coil_breakout_vol_mult` | 1.5 | 2026-07-04 신규(MIK 보정). 돌파 거래량 확장 판정을 `vols[i] ≥ ma50×breakout_vol_mult(1.4)` **OR** `vols[i] ≥ 코일평균거래량×coil_breakout_vol_mult(1.5)` 중 하나로 완화. 근거: 최근 IPO·저유동주는 상장초기 고거래량이 50일선을 부풀려 실제 돌파 확장이 MA50 기준으론 "조용"해 보임 — 마른 코일 자체를 기준선으로 삼으면 이 왜곡을 보정. `coil=None`(기존 호출부) 시 (b) 경로는 비활성이라 기존 동작 완전 보존. |
