...
```
> 구현 참고: detector_verdict 는 `status_at_b1`(b1 시점 status)을 담고, 수축 깊이는 `contractions` 축에, 검출기 돌파일은 `breakout.detector_flags`에 있다(예시의 note/depth/breakout_days 키는 설명용).
