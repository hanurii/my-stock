...
### 4.2 앵커링 — 선반 고점·컵 바닥·왼쪽 테두리 (파워플레이 트릭 재사용)

가장 신뢰할 수 있는 앵커는 **최근의 선반**이다(파워플레이가 최근 깃발에
앵커링하는 것과 동일). 순서대로:

1. **치트 선반 고점(shelf_high) = 피벗(돌파 매수점)** = **"뒤에 눌림이 확인된
   가장 높은 고점"**.
   ...
   > 으로 한정한다.
   - 구현: 피벗 후보 = `min(이후 저가들) ≤ 고가 × (1 − min_shelf_pullback/100)` 를
     만족하는 고점(= 그 뒤로 `min_shelf_pullback`%(기본 3) 이상 눌린 고점). 후보 중
     **가장 높은 고가**가 `shelf_high`. (후보가 없으면 구간 최고 고가로 폴백.)
2. **컵 바닥(cup_low)** = `shelf_high` *이전* 구간 `[0, shelf_high_idx)` 의 최저 저점.
3. **왼쪽 테두리(left_rim_high, 옛 고점)** = `cup_low` *이전* 구간 `[0, cup_low_idx]`
   의 최고 고가.
...
- `reason`은 `pattern_detected=false`인 모든 경로에서 채워진다
  (no_data / no_series / base_too_short / cup_too_shallow / cup_too_deep /
  cup_too_short / shelf_too_short / shelf_too_long / shelf_too_loose /
  shelf_too_high_in_cup / volume_not_drying / eval_error:*).
...
- 실제 트렌드 통과 종목으로 1회 풀런 → 상태 분포 합리성 육안 확인(대부분
  미검출/forming, 검출은 소수).
- `--ticker`로 알려진 컵 베이스 종목 하나를 찍어 컵 깊이·선반 위치·피벗이
  차트와 맞는지 대조.
