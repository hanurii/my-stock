...
(거래량 마름 `volume_dryup_ratio`는 §4.5에서 **소프트 신호**로 둔다 — 하드 게이트 아님.)

### 4.5 하드 게이트 vs 보고용 소프트 신호 (핵심 변경)
`pattern_detected = true` ⟺ **3개 하드 게이트** 모두 충족:
...
- `volume_dryup_ratio`("깃발 거래량 마름") — **소프트**(프로토타입 검증: 케이엠더블유는
  깃발 거래량이 안 말랐는데도 책은 파워 플레이로 봄 → 하드 게이트면 오탈락). 거래량
  신호는 §4.6 돌파 대량거래(`breakout_vol_mult`)가 실질 담당.
- `tightness_pct` — 종전대로 보고용.

→ reason 집합에서 **`not_quiet_before_pole`·`pole_volume_weak`·`volume_not_drying`
제거**(더는 게이트 아님). "말기 베이스 제외" 보호는 1단계 RS≥80 + 깃대 ≤14주 +
타이트 깃발이 대신함.

**프로토타입 검증(5개 책 예시, FDR 실데이터)**: 위 설계로 **4/5** — 케이엠더블유·
화인베스틸·BBY는 피벗 직전 며칠 내 `pattern_detected=True`(actionable→breakout),
다우데이타는 의도대로 미검출. **티앤엘은 미검출(중첩 깃발 — §9 후속)**.
...
- `reason` 가능값: `no_data / no_series / base_too_short / pole_gain_too_small /
  flag_too_short / flag_too_long / flag_too_deep / eval_error:*`
  (← `not_quiet_before_pole`·`pole_volume_weak`·`volume_not_drying` 삭제).
...
- **5개 책 예시 회귀(`test_power_play_examples.py`)**: 각 종목의 피벗 구간
  ...
  **피벗 ±윈도(예: 피벗−7 ~ 피벗+3 거래일)** 안의 매일을 as-of(`series[:i+1]`)로
  `evaluate_power_play` 실행하여 **그 윈도 안에 `pattern_detected==True`인 날이 있는지**
  로 판정한다(실매매도 돌파 *당일*이 아니라 직전 actionable로 잡으므로). 기대:
  - 케이엠더블유(2019-07-10)·화인베스틸(2020-05-25)·BBY(1997-12초): 윈도 내 검출
    AND 그 날 `status ∈ {actionable, breakout}` AND 피벗이 실제 깃발 천장 근방.
  - 다우데이타(2020-05-07): 윈도 내 **미검출**(폴 시작형, 의도적).
  - **티앤엘(2021-06-16)**: 현재 설계로 **미검출이 정상**(중첩 깃발 — §9 후속). 테스트는
    이를 **`xfail`(알려진 한계)** 로 표기해 추적하고, §9 후속이 끝나면 green 전환되게 둔다.
  - 픽스처가 케이엠/화인/BBY/다우 중 하나라도 기대와 다르면 **로직 우선 점검**(임계값
    땜질 금지), 설계 자체가 어긋나면 spec으로 회귀.
...
## 9. 미해결/후속
- **(1순위) 티앤엘식 "중첩 깃발 / 최종 타이트 수축" 식별**: 넓은 베이스(41일·24%)
  안의 *마지막 가장 타이트한 수축*을 분리해 피벗으로 잡는 VCP식 로직. 현재는 최고가/
  전체 범위만 봐 중첩 구조를 못 집어냄(미너비니 "알고리즘은 VCP를 시각적으로 못
  잡는다"의 핵심부, [[vcp-book-examples-gaps]]와 동일 약점). 완료 시 §8 티앤엘 xfail→green.
- **깃대 측정 looseness**: `max_flagpole_days=70`이 최저점을 너무 깊게 잡아 케이엠더블유
  깃대가 303%(책 read는 148%)로 과대. 검출은 되나 수치가 헐거움 — 폴 시작점을 "급등
  시작"으로 더 정교히 잡는 튜닝 후속.
- "폴 시작형"(다우데이타식, 조용한 베이스 돌파 매수) 별도 패턴으로 후속 가능.
- 소프트 신호(조용·깃대거래량·dryup)를 순위 점수로 합성하는 랭킹은 후속.
