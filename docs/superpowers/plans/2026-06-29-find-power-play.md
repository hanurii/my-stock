...
## Task 6: breakout 피벗 수정 (신고가 돌파가 잡히도록)

Task 3 리뷰에서 드러난 설계 결함: `find_flagpole`이 피벗을 "구간 최고 고가"로
잡아, 교과서적 신고가 돌파일에는 돌파 봉 자신이 피벗이 되어 `breakout`이 영영
발화하지 못했다(spec §4.2 개정). 피벗을 **"그 뒤로 `min_flag_pullback`% 이상
눌린 가장 높은 고점"**(돌파 봉처럼 뒤에 눌림이 없는 봉은 후보 제외)으로 바꾼다.
find-vcp가 피벗을 "확정된 스윙 고점(진행 중 봉 제외)"으로 잡는 것과 같은 원리.

**Files:**
- Modify: `scripts/canslim_lib/power_play.py` (`find_flagpole` 시그니처·로직, `DEFAULT_PARAMS`, `evaluate_power_play`의 `find_flagpole` 호출)
- Test: `tests/test_power_play.py`

**Interfaces:**
- `find_flagpole(highs, lows, max_flagpole_days, min_flag_pullback=None) -> dict`
  — 4번째 인자 추가. `None`이면 기존 동작(구간 최고 고가)로 **하위호환**(Task 1의
  3-인자 호출·테스트 무변경). 값이 주어지면 "뒤에 눌림 확인된 고점" 규칙.
- `DEFAULT_PARAMS`에 `"min_flag_pullback": 3.0` 추가.

- [ ] **Step 1: 실패 테스트 작성** (`tests/test_power_play.py`에 추가)
...
Run: `python -m pytest tests/test_power_play.py::test_find_flagpole_excludes_fresh_breakout_bar tests/test_power_play.py::test_evaluate_breakout_with_physical_new_high -v`
Expected: FAIL — 현재는 피벗이 신고가(115)로 잡혀 첫 테스트는 `110 != 115`, 둘째는 `status != breakout`.
...
`find_flagpole`을 다음으로 교체(빈 입력 가드는 유지):
```python
...
`evaluate_power_play` 안의 `find_flagpole` 호출에 인자 추가:
```python
    fp = find_flagpole(highs, lows, p["max_flagpole_days"], p["min_flag_pullback"])
```

- [ ] **Step 4: Task 3의 breakout 테스트를 물리적 값으로 정정**

`test_status_breakout_on_pivot_break_with_volume`의 합성 봉을 비물리적
(`close>high`)에서 물리적 값으로 바꾼다: `high=113.0, close=112.0, low=111.0`.
(피벗이 이제 깃발 천장 111.1에 고정되므로 종가 112 > 피벗으로 정상 돌파.)
그리고 Task 2 리뷰 Minor 정리: `test_status_actionable_near_pivot_with_dryup`의
주석을 실제 피벗(≈111.1) 기준으로 정정.

- [ ] **Step 5: 전체 테스트 실행**

Run: `python -m pytest tests/test_power_play.py -v`
Expected: PASS (기존 18 + 신규 2 = 20). 합성 픽스처가 의도 reason/status를
못 내면 **구현 로직이 아니라 픽스처 수치만** 조정(사유 기록). find_flagpole의
3-인자 호출 테스트(Task 1)는 무변경 통과해야 한다.

- [ ] **Step 6: Commit**

```bash
git add scripts/canslim_lib/power_play.py tests/test_power_play.py
git commit -m "fix(power-play): 피벗=뒤에 눌림 확인된 고점 — 신고가 돌파 breakout 인식"
```
...
**참고(엣지, Task 6 반영):** `find_flagpole`의 피벗은 "뒤에 `min_flag_pullback`% 이상 눌린 가장 높은 고점"이라, 신고가로 막 돌파한 마지막 봉은 피벗 후보에서 빠지고 직전 깃발 천장이 피벗으로 남아 `breakout`이 정상 발화한다. 아직 깃발 없이 신고가만 갱신 중이면(뒤 눌림 없음) 피벗 후보가 없어 구간 최고 고가로 폴백 → `flag_len` 짧아 `flag_too_short`로 미검출. `status` 분기의 `last_close < flag_low`는 `flag_low`가 깃발 구간 최저 저점이므로 종가가 그 아래로 마감한 붕괴를 잡는다.
